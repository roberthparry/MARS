...
/**
 * @brief a struct to represent a span of time, e.g. for representing the difference between two datetimes, or for
 * representing a duration to add to a datetime.
 */
...
/**
 * @brief calculate the date of Easter Sunday for a given year and initialise a datetime structure with that date. The
 * algorithm used is the Anonymous Gregorian algorithm, which is a well-known method for calculating the date of Easter
 * Sunday in the Gregorian calendar. The algorithm takes the year as input and calculates the month and day of Easter
 * Sunday based on a series of mathematical operations. The resulting month and day are then used to initialise the
 * datetime structure. Easter Sunday is the first Sunday after the first full moon on or after 21 March, which is the
 * vernal equinox.
 * @param dttm the datetime structure to initialise with the date of Easter Sunday. The year field of this structure
 * will be set to the input year, and the month and day fields will be set to the calculated month and day of Easter
 * Sunday. The hour, minute, and second fields will be set to 0, and the Julian Day and Julian Day Number fields will be
 * set to their uninitialised values (DBL_MAX and LONG_MAX, respectively). If the input year is less than 1 or greater
 * than 9999, the function will return NULL to indicate an error.
 * @param year the year for which to calculate the date of Easter Sunday. This should be a positive integer between 1
 * and 9999, inclusive. If the input year is outside this range, the function will return NULL to indicate an error.
 * @return a pointer to the initialised datetime structure with the date of Easter Sunday, or NULL if the input year is
 * invalid.
 */
...
/**
 * @brief calculate the timezone offset in hours for a given datetime. This function uses the system's time zone
 * information to determine the offset. If the system's time zone information is not available or if the datetime is not
 * properly initialised, the function will return DBL_MAX.
...
/**
 * @brief convert a datetime to GMT (UTC) by subtracting the local timezone offset. This function will modify the input
 * datetime in place and return a pointer to it. If the conversion fails for any reason (e.g. if the input datetime is
 * not properly initialised), the function will return NULL.
...
/**
 * @brief get the year of a datetime. If the year is not initialised, it will be calculated from the Julian Day Number
 * or the Julian Day.
...
/**
 * @brief get the month of a datetime. If the month is not initialised, it will be calculated from the Julian Day Number
 * or the Julian Day.
 * @param dttm the datetime to get the month from.
 * @return the month of the datetime, or 0 if it is not initialised and cannot be calculated. Note that month 0 is not
 * valid, so it can be used as a sentinel value for uninitialised month.
 */
month_t datetime_month(const datetime_t *dttm);

/**
 * @brief get the day of a datetime. If the day is not initialised, it will be calculated from the Julian Day Number or
 * the Julian Day.
 * @param dttm the datetime to get the day from.
 * @return the day of the datetime, or 0 if it is not initialised and cannot be calculated. Note that day 0 is not
 * valid, so it can be used as a sentinel value for uninitialised day.
 */
uint8_t datetime_day(const datetime_t *dttm);

/**
 * @brief get the hour of a datetime. If the hour is not initialised, it will be calculated from the Julian Day Number
 * or the Julian Day.
 * @param dttm the datetime to get the hour from.
 * @return the hour of the datetime, or 0 if it is not initialised and cannot be calculated. Note that hour 0 is not
 * valid, so it can be used as a sentinel value for uninitialised hour.
 */
uint8_t datetime_hour(const datetime_t *dttm);

/**
 * @brief get the minute of a datetime. If the minute is not initialised, it will be calculated from the Julian Day
 * Number or the Julian Day.
 * @param dttm the datetime to get the minute from.
 * @return the minute of the datetime, or 0 if it is not initialised and cannot be calculated. Note that minute 0 is not
 * valid, so it can be used as a sentinel value for uninitialised minute.
 */
uint8_t datetime_minute(const datetime_t *dttm);

/**
 * @brief get the second of a datetime. If the second is not initialised, it will be calculated from the Julian Day
 * Number or the Julian Day.
 * @param dttm the datetime to get the second from.
 * @return the second of the datetime, or 0.0 if it is not initialised and cannot be calculated. Note that second 0.0 is
 * not valid, so it can be used as a sentinel value for uninitialised second.
 */
...
/**
 * @brief get the weekday of a datetime. If the weekday is not initialised, it will be calculated from the Julian Day
 * Number or the Julian Day.
 * @param dttm the datetime to get the weekday from.
 * @return the weekday of the datetime, or 0 if it is not initialised and cannot be calculated. Note that weekday 0 is
 * not valid, so it can be used as a sentinel value for uninitialised weekday. The returned weekday is in the range
 * [Sunday, Saturday] (1-7).
 */
...
/**
 * @brief check if a datetime is less than or equal to another datetime (i.e. represents an earlier or the same point in
 * time).
 * @param dttm1 the first datetime to compare.
 * @param dttm2 the second datetime to compare.
...
/**
 * @brief check if a datetime is greater than or equal to another datetime (i.e. represents a later or the same point in
 * time).
 * @param dttm1 the first datetime to compare.
 * @param dttm2 the second datetime to compare.
...
/**
 * @brief compare two datetimes. The result is -1 if dttm1 is less than dttm2, 0 if they are equal, and 1 if dttm1 is
 * greater than dttm2. This is a common pattern for comparison functions in C,
 * @param dttm1 the first datetime to compare.
 * @param dttm2 the second datetime to compare.
 ...
 *        because we need to know how many days are in the target month to handle edge cases like adding 1 month to
 * January 31st.
...
/**
 * @brief get the number of days in the month of a datetime. If the month or year is not initialised, it will be
 * calculated from the Julian Day Number or the Julian Day.
 * @param dttm the datetime to get the number of days in its month from.
 * @return the number of days in the month of the datetime, or 0 if the month or year is not initialised and cannot be
 * calculated. Note that 0 is not a valid number of days in a month,
 */
unsigned short datetime_days_in_this_month(const datetime_t *dttm);

/**
 * @brief check if a datetime is in daylight saving time. This is a bit tricky because it depends on the local time zone
 * and the rules for daylight saving time, which can change over time and vary by location.
 * @param dttm the datetime to check.
 * @return true if the datetime is in daylight saving time, false otherwise. If the datetime is in an uninitialised
 * state that cannot be calculated, it returns false.
 */
...
/**
 * @brief add a number of months to a datetime and return the current datetime. The original datetime is modified. Note
 * that adding months is more complex than adding days or weeks because months have different lengths and there are edge
 * cases like adding 1 month to January 31st.
...
/**
 * @brief add a number of years to a datetime and return the current datetime. The original datetime is modified. Note
 * that adding years is more complex than adding days or weeks because of edge cases like adding 1 year to February 29th
 * on a leap year.
 ...
 * @return the modified datetime if successful, or NULL if the datetime is in an uninitialised state that cannot be
 * calculated.
 */
 ...
 *
 * This function subtracts a datetime_span_t from a datetime. If the datetime is in an uninitialised state that cannot
 * be calculated, it will return NULL.
 *
 * @param dttm the datetime to subtract from
 * @param span the datetime_span_t to subtract
 * @return the datetime after subtracting the datetime_span_t, or NULL if the datetime is in an uninitialised state that
 * cannot be calculated.
 */
datetime_t *datetime_sub_span(datetime_t *dttm, const datetime_span_t *span);

/**
 * @brief calculate a hash code for a datetime. This can be useful for using datetimes as keys in hash tables or for
 * quickly comparing datetimes. The hash code is calculated based on the year, month, day, hour, minute, and second of
 * the datetime. If the datetime is in an uninitialised state that cannot be calculated, the function will try to
 * calculate the year, month, day, hour, minute, and second from the Julian Day Number or the Julian Day before
 * calculating the hash. Note that this means that if you have two datetimes that are in an uninitialised state but
 * represent the same point in time (e.g. they have the same Julian Day), they will have the same hash code after this
 * function is called.
...
/**
 * @brief calculate the duration between two datetimes in days, and optionally fill in a datetime_span_t structure with
 * the duration in years, months, days, hours, minutes, and seconds. The duration is calculated as dttm1 - dttm2, so it
 *        will be positive if dttm1 is later than dttm2, negative if dttm1 is earlier than dttm2, and zero if they are
 * equal. If either datetime is in an uninitialised state that cannot be calculated, the function returns DBL_MAX to
 * indicate that the duration cannot be calculated. If the span parameter is not NULL, it will be filled with the
 * duration in years, months, days, hours, minutes, and seconds. The span will always represent a positive duration
 * (i.e. years, months, days, hours, minutes, and seconds will all be non-negative), and the sign of the overall
 * difference will be reflected in the return value of the function (positive for dttm1 > dttm2, negative for dttm1 <
 * dttm2).
 * @param dttm1 the first datetime to compare.
 * @param dttm2 the second datetime to compare.
 * @param span an optional pointer to a datetime_span_t structure to fill with the duration in years, months, days,
 * hours, minutes, and seconds. If NULL, the span will not be filled.
 * @return the difference between dttm1 and dttm2 in days, or DBL_MAX if the difference cannot be calculated due to
 * uninitialised datetimes.
 */
 ...
 * @param format the format string to use for formatting the datetime. See the description above for the supported
 * placeholders.
 * @return a newly allocated C string containing the formatted datetime. The caller is responsible for freeing the
 * returned string. If allocation fails or the datetime is not initialised, returns NULL.
 */
...
/**
 * @brief calculate the sunrise or sunset time for a given date and location. The sunrise/sunset time is calculated
 * using the algorithm described in the NOAA Solar Calculator, which is based on the equations from the Astronomical
 * Algorithms book by Jean Meeus. The function takes the Julian Day Number for the date, the latitude and longitude of
 * the location, and a boolean indicating whether to calculate sunrise or sunset. The function returns the sunrise or
 * sunset time in GMT as a decimal number of hours (e.g. 6.5 for 6:30 AM). If the sunrise/sunset time cannot be
 * calculated for the given date and location (e.g. polar night), the function returns -1.0 to indicate that the sun
 * never rises, -2.0 to indicate that the sun never sets.
 ...
 * @return the sunrise or sunset time in GMT as a decimal number of hours (e.g. 6.5 for 6:30 AM). If the sunrise/sunset
 * time cannot be calculated for the given date and location (e.g. polar night), the function returns -1.0 to indicate
...
/**
 * @brief initialise a datetime object with the sunrise time for a given date and location. This function is a wrapper
 * around datetime_init_sun_time() that calls it with the isSunrise parameter set to true to calculate the sunrise time.
 *        The date is specified by the Julian Day Number, and the location is specified by the latitude and longitude.
 * The time zone offset is also taken into account to adjust the time to the local time zone (inclusive of daylight
 * saving time). If the calculated time is less than 0, it means that the sunrise occurs on the previous day, so we add
 * 24 hours to the time and subtract one day from the datetime object.
 ...
 * @return a pointer to the initialised datetime object. If the sunrise time cannot be calculated for the given date and
 * location (e.g. polar night), the time components of the datetime object will be set to 0, and the function will
 * return the datetime object with the date set to the given Julian Day Number.
 */
...
/**
 * @brief initialise a datetime object with the sunset time for a given date and location. This function is a wrapper
 * around datetime_init_sun_time() that calls it with the isSunrise parameter set to false to calculate the sunset time.
 *        The date is specified by the Julian Day Number. The location is specified by the latitude and longitude. The
 * time zone offset is also taken into account to adjust the time to the local time zone (inclusive of daylight saving
 * time). If the calculated time is less than 0, it means that the sunset occurs on the next day, so we subtract 24
 * hours from the time and add one day to the datetime object.
 ...
 * @param timeZoneOffset the time zone offset in hours to adjust the calculated time to local time.
 * @return a pointer to the initialised datetime object. If the sunset time cannot be calculated for the given date and
 * location (e.g. polar night), the time components of the datetime object will be set to 0, and the function will
 * return the datetime object with the date set to the given Julian Day Number.
 */
 ...
 * @return @p dttm on success, or @c NULL on allocation/initialisation failure.
 */
datetime_t *datetime_init_next_sunrise_checked(datetime_t *dttm, long julianDayNumber, double latitude,
...
/**
 * @brief set the time components of a datetime object to the sunrise time for its date and a given location. This
 * function is a wrapper around datetime_set_sun_time() that calls it with the isSunrise parameter set to true. The
 * location is specified by the latitude and longitude. The time zone offset is used to adjust the calculated time to
 * the local time zone.
...
/**
 * @brief set the time components of a datetime object to the sunset time for its date and a given location. This
 * function is a wrapper around datetime_set_sun_time() that calls it with the isSunrise parameter set to false. The
 * location is specified by the latitude and longitude. The time zone offset is used to adjust the calculated time to
 * the local time zone.
 * @param dttm the datetime object to set the time components of.
 * @param latitude the latitude of the location to calculate the sunset time for.
...
/**
 * @brief get the moon phase for a given datetime object. This function calculates the Julian Day Number for the given
 * datetime object, and then uses that Julian Day Number to calculate the moon phase using the
 * datetime_moon_phase_on_jdn() function. The moon phase is returned as a value of the moon_phase_t enum, which
 * represents the different phases of the moon (e.g. New Moon, Waxing Crescent, First Quarter, etc.).
 * @param dttm the datetime object to get the moon phase for. The date components of the datetime object should be set
 * to the desired date, and the time components can be set to any value (they will not affect the moon phase
 * calculation).
 * @return the moon phase for the given datetime object. The moon phase is returned as a value of the moon_phase_t enum,
 * which represents the different phases of the moon (e.g. New Moon, Waxing Crescent, First Quarter, etc.). If the
 * datetime object is not initialised (i.e. its year component is SHRT_MAX), the function will return DT_NewMoon as a
 * default value.
 */
...
/**
 * @brief find the next datetime with a specific moon phase after a given datetime. This function calculates the moon
 * phase for the given datetime object, and then iterates forward in time until it finds a datetime with the specified
 * moon phase. The function returns a pointer to the next datetime object with the specified moon phase. If the given
 * datetime object is not initialised (i.e. its year component is SHRT_MAX), the function returns NULL.
 * @param dttm the datetime object to start the search from. The date components of the datetime object should be set to
 * the desired starting date, and the time components can be set to any value (they will not affect the moon phase
 * calculation). If the datetime object is not initialised (i.e. its year component is SHRT_MAX), the function will
 * return NULL.
 * @param phase the moon phase to search for. This should be a value of the moon_phase_t enum, which represents the
 * different phases of the moon (e.g. New Moon, Waxing Crescent, First Quarter, etc.).
 * @return a pointer to the next datetime object with the specified moon phase, or NULL if no such datetime exists or if
 * the input datetime is not initialised. The caller is responsible for freeing the returned datetime object when it is
 * no longer needed.
 */
datetime_t *datetime_next_moon_phase(const datetime_t *dttm, moon_phase_t phase);

/**
 * @brief find the next datetime with a specific weekday after a given datetime. This function calculates the weekday
 * for the given datetime object, and then iterates forward in time until it finds a datetime with the specified
 * weekday. The function returns a pointer to the next datetime object with the specified weekday. If the given datetime
 * object is not initialised (i.e. its year component is SHRT_MAX), the function returns NULL.
 * @param dttm the datetime object to start the search from. The date components of the datetime object should be set to
 * the desired starting date, and the time components can be set to any value (they will not affect the weekday
 * calculation). If the datetime object is not initialised (i.e. its year component is SHRT_MAX), the function will
 * return NULL.
 * @param weekday the weekday to search for (Sunday, Monday, ..., Saturday).
 * @return a pointer to the next datetime object with the specified weekday, or NULL if no such datetime exists or if
 * the input datetime is not initialised. The caller is responsible for freeing the returned datetime object when it is
 * no longer needed.
 */
