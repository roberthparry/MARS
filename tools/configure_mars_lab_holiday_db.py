...
DB_PATH_ENV = "MARS_HOLIDAY_DB_PATH"
DB_KEY_ENV = "MARS_HOLIDAY_DB_KEY"
...
def config_path() -> Path:
    return mars_home() / "config" / "holiday-db.env"


def default_db_path() -> Path:
    return mars_home() / "holiday" / "mars_holiday_rules.db"
...
def write_config(path: Path, db_path: str, db_key: str) -> None:
    ...
    body = "\n".join(
        [
            "# Generated by tools/configure_mars_lab_holiday_db.py",
...
def build_holiday_database(db_path: Path, db_key: str) -> None:
    ...
    script = "\n".join(
        [
            ...
            f".read {HOLIDAY_RULES_SQL.relative_to(ROOT)}",
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument("--db-path", default="", help="Holiday DB path to store.")
    parser.add_argument("--db-key", default="", help="Holiday DB key to store.")
    parser.add_argument(
        "--noninteractive",
        action="store_true",
