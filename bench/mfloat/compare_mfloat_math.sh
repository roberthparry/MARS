...
ref="${1:-HEAD}"
tmpdir="$(mktemp -d /tmp/mars_mfloat_math_compare.XXXXXX)"
cleanup() {
    rm -rf "$tmpdir"
}
trap cleanup EXIT
...
cp "bench/mfloat/bench_mfloat_math.c" "$tmpdir/repo/bench/mfloat/bench_mfloat_math.c"

echo
echo "Current tree"
make bench_mfloat_math >/dev/null
build/release/bench/bench_mfloat_math

echo
echo "Baseline (${ref})"
make -C "$tmpdir/repo" bench_mfloat_math >/dev/null
"$tmpdir/repo/build/release/bench/bench_mfloat_math"
