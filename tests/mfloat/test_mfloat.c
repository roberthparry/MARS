...
void test_string_parser_edge_cases(void)
{
    mfloat_t *sci = mf_create_string("  -12.5e-1  ");
    mfloat_t *inf = mf_create_string("INF");
    mfloat_t *ninf = mf_create_string("-INF");
    mfloat_t *nanv = mf_create_string("NAN");

    ASSERT_NOT_NULL(sci);
    ASSERT_NOT_NULL(inf);
    ASSERT_NOT_NULL(ninf);
    ASSERT_NOT_NULL(nanv);

    print_mfloat_value("scientific parse", sci);
    print_mfloat_error_check_decimal("mf_create_string(\"  -12.5e-1  \")", sci, "-1.25");
    ASSERT_TRUE(mfloat_meets_precision(sci, "-1.25", 0));

    print_mfloat_value("INF parse", inf);
    ASSERT_TRUE(mf_is_inf(inf));
    ASSERT_EQ_LONG((long)mf_get_sign(inf), 1);

    print_mfloat_value("-INF parse", ninf);
    ASSERT_TRUE(mf_is_inf(ninf));
    ASSERT_EQ_LONG((long)mf_get_sign(ninf), -1);

    print_mfloat_value("NAN parse", nanv);
    ASSERT_TRUE(mf_is_nan(nanv));
...
void test_extended_math_wrappers(void)
{
    ...
    print_mfloat_error_check("tan(0.5) mfloat error", c,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        c,
        "0.",
        1));
    ...
    print_mfloat_error_check("atan(0.5) mfloat error", c,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        c,
        "0.",
        1));
    ...
    print_mfloat_error_check("erf(0.5) mfloat error", c,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        c,
        "0.",
        1));
    ...
    print_mfloat_error_check("erfc(0.5) mfloat error", c,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        c,
        "0.",
        1));
...
void test_tetragamma_function(void)
{
    ...
    ASSERT_NOT_NULL(x);
    expected = mf_create_string(
        "-0.");
    ...
    print_mfloat_error_check("tetragamma(2.345) mfloat error", x,
                             "-0.");
    ASSERT_TRUE(mfloat_matches_expected_bits(
        x, expected, 1, TEST_MFLOAT_MATHS_PRECISION - 8u));
...
void test_digamma_function(void)
{
    ...
    print_mfloat_error_check("digamma(2.345) mfloat error", x,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        x,
        "0.",
        1));
...
void test_trigamma_function(void)
{
    ...
    print_mfloat_error_check("trigamma(2.345) mfloat error", x,
                             "0.");
    ASSERT_TRUE(mfloat_meets_precision(
        x,
        "0.",
        1));
...
void test_remaining_special_mfloat_functions(void)
{
    ...
    ASSERT_EQ_INT(mf_set_string(y, "2.5"), 0);
    ASSERT_EQ_INT(mf_gamma(y), 0);
    ASSERT_EQ_INT(mf_gammainv(y), 0);
    ...
    print_mfloat_error_check("e1(1) mfloat error", one,
                             "0.");
    expected_calc = mf_create_string(
        "0.");
    ASSERT_NOT_NULL(expected_calc);
    ASSERT_TRUE(mfloat_matches_expected_bits(
        one, expected_calc, 1, TEST_MFLOAT_MATHS_PRECISION - 8u));
    mf_free(expected_calc);
    expected_calc = NULL;
...
void test_log_family(void)
{
    ...
    ASSERT_EQ_INT(mf_set_long(a, 1000), 0);
    ASSERT_EQ_INT(mf_log10(a), 0);
    print_mfloat_error_check_decimal("log10(1000) mfloat error", a, "3");
    ASSERT_TRUE(mfloat_meets_precision(a, "3", 0));
