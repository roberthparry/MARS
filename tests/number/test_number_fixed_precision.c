...
void run_number_fixed_precision_tests(void)
{
...

    {
        ...
        number_t want_w0 = num_create_from_qcomplex(qc_productlog(qc_from_string("0.2 + 0.1i")));
        number_t want_sin_i = num_create_from_qcomplex(qc_sin(qc_from_string("0 + 1i")));
        number_t want_cos_i = num_create_from_qcomplex(qc_cos(qc_from_string("0 + 1i")));
        number_t want_tan_i = num_create_from_qcomplex(qc_tan(qc_from_string("0 + 1i")));
        number_t want_sinh_i = num_create_from_qcomplex(qc_sinh(qc_from_string("0 + 1i")));
        number_t want_cosh_i = num_create_from_qcomplex(qc_cosh(qc_from_string("0 + 1i")));
        number_t want_tanh_i = num_create_from_qcomplex(qc_tanh(qc_from_string("0 + 1i")));
        ...
        ASSERT_NUMBER_EQ(sin_i, want_sin_i);
        ASSERT_NUMBER_EQ(cos_i, want_cos_i);
        ASSERT_NUMBER_EQ(tan_i, want_tan_i);
        ASSERT_NUMBER_EQ(sinh_i, want_sinh_i);
        ASSERT_NUMBER_EQ(cosh_i, want_cosh_i);
        ASSERT_NUMBER_EQ(tanh_i, want_tanh_i);
        ASSERT_NUMBER_EQ(qc_w0, want_w0);
        ...
        num_destroy(&qc_w0);
        num_destroy(&want_w0);
        num_destroy(&want_sin_i);
        num_destroy(&want_cos_i);
        num_destroy(&want_tan_i);
        num_destroy(&want_sinh_i);
        num_destroy(&want_cosh_i);
        num_destroy(&want_tanh_i);
        num_destroy(&zero);
