...
#define ASSERT_EQ(actual, expected) \
    TEST_ASSERT_LONG_EQ((long)(actual), (long)(expected))

#define ASSERT_NOT_NULL(ptr) \
    TEST_ASSERT_NOT_NULL((ptr))

#define ASSERT_OK(expr) \
    TEST_ASSERT_INT_EQ((expr), 0)

#define ASSERT_STREQ(actual, expected)                                     \
    do {                                                                   \
        const char *_actual_text = (actual);                               \
        const char *_expected_text = (expected);                           \
        if (!_actual_text || !_expected_text ||                            \
            strcmp(_actual_text, _expected_text) != 0) {                   \
            test_set_failure_detailf("expected \"%s\", got \"%s\"",        \
                                     _expected_text ? _expected_text : "(null)", \
