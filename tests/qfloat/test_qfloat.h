...
#define TEST_ASSERT_QFLOAT_CLOSE(got_value, want_value)                                                         \
    do {                                                                                                               \
        qfloat_t test_qfloat_got__ = (got_value);                                                                \
        qfloat_t test_qfloat_want__ = (want_value);                                                            \
        TEST_ASSERT_VALID_NAMED("qfloat-close", &test_qfloat_got__, &test_qfloat_want__);                       \
    } while (0)

#define TEST_ASSERT_QFLOAT_CLOSE_TOL(got_value, want_value, rel_tol_value)                                      \
    do {                                                                                                               \
        qfloat_t test_qfloat_got__ = (got_value);                                                                \
        qfloat_t test_qfloat_want__ = (want_value);                                                            \
        if (!test_assert_qfloat_close_tol(test_qfloat_got__, test_qfloat_want__, (rel_tol_value), __FILE__,     \
                                          __LINE__))                                                                   \
            return;                                                                                                    \
    } while (0)
