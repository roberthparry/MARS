...
static void test_partial_get_borrowed(void)
{
    ...
    if (p1 != p2) {
        printf(C_BOLD C_RED "FAIL" C_RESET " expr_get_deriv not cached %s:%d:1\n", __FILE__, __LINE__);
        TEST_FAIL();
    } else {
...
static void test_partial_to_string_sin_xy(void)
{
    ...
    if (str_eq(s, "{ ln(y) + y·cos(xy) | y = 2, x = 1 }"))
        ...
        to_string_fail(__FILE__, __LINE__, 1, "∂(sin(xy)+x·ln(y))/∂x (EXPR)", s,
                       "{ ln(y) + y·cos(xy) | y = 2, x = 1 }");
    free(s);
    ...
    if (str_eq(s, "{ cos(xy) - xy·sin(xy) + 1/y | x = 1, y = 2 }"))
        ...
        to_string_fail(__FILE__, __LINE__, 1, "∂²(sin(xy)+x·ln(y))/∂x∂y (EXPR)", s,
                       "{ cos(xy) - xy·sin(xy) + 1/y | x = 1, y = 2 }");
    free(s);
