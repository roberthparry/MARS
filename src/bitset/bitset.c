...
{
    if (needed_words <= bs->word_count)
        return true;
    size_t new_count = bs->word_count ? bs->word_count * 2 : BITSET_INIT_WORDS;
    while (new_count < needed_words)
        new_count *= 2;
    uint64_t *new_words = realloc(bs->words, new_count * sizeof(uint64_t));
    if (!new_words)
        return false;
    memset(new_words + bs->word_count, 0, (new_count - bs->word_count) * sizeof(uint64_t));
    bs->words = new_words;
...
bitset_t *bitset_create(size_t initial_capacity)
{
    ...
    size_t init_words = initial_capacity ? words_for_bits(initial_capacity) : BITSET_INIT_WORDS;
...
bool bitset_test(const bitset_t *bs, size_t index)
{
    ...
    pthread_mutex_lock((pthread_mutex_t *)&bs->mutex);
    size_t word = index / BITSET_WORD_BITS;
    bool result = (word < bs->word_count) && ((bs->words[word] >> (index % BITSET_WORD_BITS)) & 1u);
    pthread_mutex_unlock((pthread_mutex_t *)&bs->mutex);
    return result;
}
...
size_t bitset_next_set(const bitset_t *bs, size_t from)
{
    ...
    while (word < bs->word_count) {
        ...
        if (w) {
            result = word * BITSET_WORD_BITS + bit + (size_t)__builtin_ctzll(w);
            break;
        }
        word++;
        bit = 0;
...
bool bitset_and(bitset_t *dst, const bitset_t *src)
{
    ...
    size_t min_words = dst->word_count < src->word_count ? dst->word_count : src->word_count;
