...
static inline void qf_put_char(
    char ch,
    char **dst,
    size_t *remaining,
    size_t *count)
...
{
    while (*str) {
        qf_put_char(*str, dst, remaining, count);
        str++;
    }
}
...
int qf_vsprintf(char *out, size_t out_size, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        ...
        /* Literal characters */
        if (*fmt != '%') {
            qf_put_char(*fmt, &dst, &remaining, &count);
            fmt++;
            continue;
        }
        ...
        while (1) {
            if      (*fmt == '+') { flag_plus = 1; fmt++; }
            else if (*fmt == ' ') { flag_space = 1; fmt++; }
            else if (*fmt == '-') { flag_minus = 1; fmt++; }
        ...
        /* ----------------------------------------------------
           %Q — scientific notation with uppercase E
           ---------------------------------------------------- */
        if (*fmt == 'Q') {
            fmt++;

            qfloat x = va_arg(ap, qfloat);

            char core[256];
            qf_to_string(x, core, sizeof(core));   /* full precision sci */
            qf_uppercase_E(core);
            ...
            pad_string(padded, sizeof(padded),
                       tmp, width, flag_minus, flag_zero,
                       0);
        ...
        /* ----------------------------------------------------
           %q — fixed-format with fallback to lowercase e
           ---------------------------------------------------- */
        if (*fmt == 'q') {
            fmt++;
               ...
               ==================================================== */
            {
                ...
                int is_integer = (lo == 0.0) && (floor(hi) == hi);
            ...
            /* Step 1: get canonical scientific form */
            char sci[128];
            ...
            /* Step 2: decide if fixed format is possible */
            if (exp10 < -6 || exp10 > 32) {

                /* ------------------------------------------------
                   FULL-PRECISION SCIENTIFIC FALLBACK
                   ------------------------------------------------ */
                char tmp[256];
                qf_to_string(x, tmp, sizeof(tmp));   /* full precision sci */
            ...
            /* ------------------------------------------------
               FIXED-FORMAT RECONSTRUCTION (unchanged)
               ------------------------------------------------ */
            char mantissa_clean[128];
            strcpy(mantissa_clean, sci);

            int negative = 0;
            char *mant = mantissa_clean;
            ...
            char all[256];
            snprintf(all, sizeof(all), "%s%s", intpart, fracpart);

            int total_digits = (int)strlen(all);
            int fixed_dp = (int)strlen(intpart) + exp10;
            ...
            if (fixed_dp <= 0) {
                *p++ = '0';
                ...
                for (int i = 0; i < fixed_dp && i < total_digits; i++)
                    *p++ = all[i];
                for (int i = total_digits; i < fixed_dp; i++)
                    *p++ = '0';
            }
            ...
            if (precision != 0 || flag_hash) {
                *p++ = '.';
                ...
                int available = total_digits - fixed_dp;

                if (precision < 0) {
                    int max_frac = available;
                    if (max_frac > 34) max_frac = 34;

                    for (int i = 0; i < max_frac; i++)
                        *p++ = (fixed_dp + i < total_digits) ? all[fixed_dp + i] : '0';
                } else {
                    int to_copy = precision < available ? precision : available;

                    for (int i = 0; i < to_copy; i++)
                        *p++ = all[fixed_dp + i];

                    if (available > precision) {
                        char next = all[fixed_dp + precision];
                        if (next >= '5') {
                            char *q = p - 1;
                            while (q >= buf2 && *q != '.' && *q != '-' && *q != '+' && *q != ' ') {
                                if (*q < '9') { (*q)++; break; }
                                *q-- = '0';
                            }
                        }
            ...
            *p = '\0';
            ...
            char padded[512] = {0};
            pad_string(padded, sizeof(padded),
                       buf2, width, flag_minus, flag_zero,
                       0);
        ...
        /* ----------------------------------------------------
           Delegate all other specifiers to snprintf
           ---------------------------------------------------- */
        {
            char fmtbuf[32];
            char tmp[256];
...
int qf_sprintf(char *out, size_t out_size, const char *fmt, ...)
{
    ...
    /* Copy the argument list for the two‑pass logic inside qf_vsprintf */
    va_list ap_copy;
    va_copy(ap_copy, ap);

    int result = qf_vsprintf(out, out_size, fmt, ap_copy);
...
int qf_printf(const char *fmt, ...)
{
    ...
    va_list ap_copy;
    va_copy(ap_copy, ap);
    int needed = qf_vsprintf(NULL, 0, fmt, ap_copy);
    va_end(ap_copy);
    ...
    va_copy(ap_copy, ap);
    int written = qf_vsprintf(buf, (size_t)needed + 1, fmt, ap_copy);
    va_end(ap_copy);
    va_end(ap);
