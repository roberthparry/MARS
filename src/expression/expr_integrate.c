...
static expr_t *negate_owned_local(expr_t *expr)
{
    expr_t *negated;
...
static expr_t *match_symbolic_wrt_factor_coeff(const expr_t *expr,
                                               const expr_t *wrt)
{
    const expr_t *left = NULL;
    const expr_t *right = NULL;
    ...
    if (expr_is_op(expr, &ops_div) && expr->a && expr->b &&
        ...
        expr_t *denom = clone_expr_local(expr->b);
                                                ...
                                                const expr_t *wrt)
{
    ...
    if (coeff) {
        if (!depends_on_wrt(left, wrt))
            return is_sub ? negate_owned_local(coeff) : coeff;
        expr_free(coeff);
    }
