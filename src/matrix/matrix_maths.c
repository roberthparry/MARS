...
static number_t mat_eval_number_scalar_number_local(void (*scalar_f)(void *out, const void *in), const number_t *input)
{
...
static matrix_t *mat_fun_apply(const matrix_t *A, void (*number_f)(void *out, const void *a),
                               void (*expr_f)(void *out, const void *a), void (*native_f)(void *out, const void *a));
static matrix_t *mat_fun_expr_structured(const matrix_t *A, void (*scalar_f)(void *out, const void *in));
static matrix_t *mat_fun_elementwise_same_type(const matrix_t *A, void (*scalar_f)(void *out, const void *in));
...
static matrix_t *mat_number_triangular_mul(const matrix_t *A, const matrix_t *B, bool upper)
{
    size_t n;
    matrix_t *C;

    if (!A || !B || A->rows != A->cols || B->rows != B->cols || A->rows != B->rows || A->elem != &number_elem ||
        B->elem != &number_elem)
        return NULL;
...
static int mat_number_add_scaled_triangular(matrix_t *F, const matrix_t *Npower, const number_t coeff, bool upper)
{
    size_t n;

    if (!F || !Npower || F->rows != F->cols || Npower->rows != Npower->cols || F->rows != Npower->rows ||
        F->elem != &number_elem || Npower->elem != &number_elem)
        return -1;
...
static matrix_t *mat_number_series_from_scaled_strict_triangular(
    const matrix_t *A, bool upper, number_t diag_value, number_t scaled_coeff0,
    number_t (*next_coeff)(const number_t prev_coeff, size_t k, void *ctx), void *ctx)
{
    size_t n;
    matrix_t *F = NULL;
    matrix_t *S = NULL;
...
static matrix_t *mat_number_unary_taylor_from_expr_upper(const matrix_t *A, expr_t *(*build_expr)(const expr_t *))
{
    size_t n;
    matrix_t *F = NULL;
    matrix_t *N = NULL;
    ...
    if (!A || !build_expr || A->rows != A->cols || A->elem != &number_elem || !mat_is_upper_triangular(A))
        return NULL;
...
static matrix_t *mat_number_unary_taylor_from_expr(const matrix_t *A, expr_t *(*build_expr)(const expr_t *))
{
    ...
    if (!A || !build_expr || A->rows != A->cols || A->elem != &number_elem || !mat_number_diagonal_equal_local(A))
        return NULL;
...
static matrix_t *mat_log_number_triangular_equal_diag_upper(const matrix_t *A)
{
    ...
    out = mat_number_series_from_scaled_strict_triangular(A, true, diag_value, inv_lambda, mat_number_log_next_coeff,
                                                          &ctx);
...
static matrix_t *mat_sqrt_number_triangular_equal_diag_upper(const matrix_t *A)
{
    ...
    out =
        mat_number_series_from_scaled_strict_triangular(A, true, diag_value, coeff0, mat_number_sqrt_next_coeff, &ctx);
...
static bool mat_expr_block_is_exact_zero(const matrix_t *A, size_t row0, size_t rows, size_t col0, size_t cols)
{
...
static matrix_t *mat_expr_permute_principal_local(const matrix_t *A, const size_t *perm)
{
    matrix_t *P;
...
static matrix_t *mat_expr_extract_block_local(const matrix_t *A, size_t row0, size_t rows, size_t col0, size_t cols)
{
    matrix_t *B;
...
static bool mat_expr_insert_block_local(matrix_t *A, size_t row0, size_t col0, const matrix_t *B)
{
...
static int mat_expr_component_partition(const matrix_t *A, size_t **perm_out, size_t *count_out)
{
    size_t n;
    size_t *perm = NULL;
    bool *seen = NULL;
...
static matrix_t *mat_fun_expr_block_diagonal(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    size_t n;
    ...
    for (size_t split = 1; split < n; ++split) {
        ...
        if (!mat_expr_insert_block_local(F, 0, 0, F11) || !mat_expr_insert_block_local(F, split, split, F22)) {
            mat_free(F);
            F = NULL;
            goto next_split;
        }
...
static matrix_t *mat_fun_expr_permuted_block_diagonal(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    size_t *perm = NULL;
    size_t comp_count = 0;
    matrix_t *P = NULL;
...
static matrix_t *mat_fun_expr_uniform_diag_offdiag(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    ...
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            mat_get(A, i, j, &entry);
            same = (i == j) ? expr_equal_exact_local(diag, entry) : expr_equal_exact_local(offdiag, entry);
            if (!same)
                goto fail;
        }
    }
    ...
    expr_t *scaled_offdiag = expr_mul_simplify_owned(expr_new_const(n_minus_one), offdiag ? offdiag : EXPR_ZERO);
...
static matrix_t *mat_fun_expr_cubic_linear_exact(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    matrix_t *A2 = NULL;
    matrix_t *A3 = NULL;
    matrix_t *out = NULL;
...
static matrix_t *mat_fun_expr_quadratic_exact(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    matrix_t *A2 = NULL;
    matrix_t *out = NULL;
    expr_t *p = NULL;
    ...
    if (mat_is_upper_triangular(A)) {
        ...
        for (size_t i = 1; i < n; ++i) {
            ...
            if (num_lt(tol, absdiff)) {
                expr_free(diag0);
                return (A->rows == 2 && A->cols == 2) ? mat_fun_expr_diagonalizable_2x2(A, scalar_f) : NULL;
            }
        }
        expr_free(diag0);
...
static matrix_t *mat_fun_apply(const matrix_t *A, void (*number_scalar_f)(void *out, const void *in),
...
static void mat_scale_number(matrix_t *A, const number_t *r)
{
    ...
    unsigned char r_raw[MATRIX_SCALAR_STORAGE_BYTES] = {0}, v[MATRIX_SCALAR_STORAGE_BYTES] = {0},
                  scaled[MATRIX_SCALAR_STORAGE_BYTES] = {0};
...
static void mat_attach_spectral_cache(matrix_t *A, const matrix_t *Vq, const number_t *evals)
{
...
static matrix_t *mat_fun_from_spectral_cache(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    size_t n = A->rows;
...
static matrix_t *mat_fun_hermitian(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    size_t n = A->rows;
    const struct elem_vtable *orig_elem = A->elem;
...
static matrix_t *mat_fun_upper_triangular(const matrix_t *A, void (*scalar_f)(void *out, const void *in))
{
    const struct elem_vtable *orig_elem = A->elem;
...
matrix_t *mat_exp(const matrix_t *A)
{
    ...
    if (A) {
        ...
        if (A->elem == &number_elem && A->rows == A->cols &&
...
matrix_t *mat_pow_int(const matrix_t *A, int n)
{
    ...
    if (!A || A->rows != A->cols)
        return NULL;
    if (!mat_elem_supports_numeric_algorithms(A) && n < 0)
        return NULL;
    size_t sz = A->rows;
    const struct elem_vtable *e = A->elem;
...
static number_t mat_scalar_principal_power(number_t base, const number_t *exponent)
{
    number_t result = num_pow(base, *exponent);

    if (!num_is_nan(result) || !num_is_real(base) || !num_is_real(*exponent) || num_get_sign(base) >= 0)
        return result;

    {
        size_t precision = num_get_prec_bits(base);
        size_t exponent_precision = num_get_prec_bits(*exponent);
        number_t complex_base;
        number_t complex_exponent;

        if (exponent_precision > precision)
            precision = exponent_precision;
        complex_base = num_as_complex_prec(base, precision);
        complex_exponent = num_as_complex_prec(*exponent, precision);
        num_destroy(&result);
        result = num_pow(complex_base, complex_exponent);
...
static matrix_t *mat_pow_diagonalizable(const matrix_t *A, const number_t *s)
{
    const size_t n = A->rows;
    number_t *eigenvalues = calloc(n, sizeof(*eigenvalues));
    matrix_t *eigenvectors = NULL;
    matrix_t *diagonal = NULL;
    matrix_t *scaled_eigenvectors = NULL;
    matrix_t *inverse_eigenvectors = NULL;
    matrix_t *result = NULL;

    if (!eigenvalues)
        return NULL;
    mat_fun_number_array_invalidate(eigenvalues, n);

    if (mat_eigendecompose(A, eigenvalues, &eigenvectors) != 0 || !eigenvectors)
        goto cleanup;

    if (!mat_is_hermitian(A)) {
        number_t repeated_tolerance = num_create_from_double(1e-20);

        for (size_t i = 0; i < n; ++i) {
            for (size_t j = i + 1; j < n; ++j) {
                number_t difference = num_sub(eigenvalues[i], eigenvalues[j]);
                number_t magnitude = num_abs(difference);
                bool repeated = num_le(magnitude, repeated_tolerance);

                num_destroy(&magnitude);
                num_destroy(&difference);
                if (repeated) {
                    num_destroy(&repeated_tolerance);
                    goto cleanup;
                }
            }
        }
    ...
    inverse_eigenvectors = mat_inverse(eigenvectors);
    if (!inverse_eigenvectors)
        goto cleanup;

    diagonal = mat_create_diagonal_with_elem(n, &number_elem);
    if (!diagonal)
        goto cleanup;

    for (size_t i = 0; i < n; ++i) {
        number_t powered = mat_scalar_principal_power(eigenvalues[i], s);

        if (num_is_nan(powered)) {
            num_destroy(&powered);
            goto cleanup;
        }
        mat_set_num_owned(diagonal, i, i, &powered);
    }

    scaled_eigenvectors = mat_mul(eigenvectors, diagonal);
    if (scaled_eigenvectors)
        result = mat_mul(scaled_eigenvectors, inverse_eigenvectors);

cleanup:
    mat_free(inverse_eigenvectors);
    mat_free(scaled_eigenvectors);
    mat_free(diagonal);
    mat_free(eigenvectors);
    mat_fun_number_array_destroy(eigenvalues, n);
    free(eigenvalues);
    return result;
}
                                            ...
                                            bool calculus)
{
    ...
    expr_t *entry = NULL;
    expr_t *plus_numerator;
    expr_t *minus_numerator;
    expr_t *plus_term;
    expr_t *minus_term;
    expr_t *result;
    ...
    if (mat_typeof(A) == MAT_TYPE_EXPR) {
        ...
        value = mat_get_num(A, row, col);
        entry = expr_new_const(value);
        num_destroy(&value);
    }
    if (!entry)
        return NULL;
    ...
    if (row == col) {
        ...
        minus_numerator = expr_sub(lambda_plus, entry);
    } else {
        plus_numerator = expr_clone(entry);
        minus_numerator = expr_neg(entry);
    }
    expr_free(entry);

    plus_term = plus_numerator ? expr_mul(lambda_plus_power, plus_numerator) : NULL;
    minus_term = minus_numerator ? expr_mul(lambda_minus_power, minus_numerator) : NULL;
    expr_free(plus_numerator);
    expr_free(minus_numerator);
    result = plus_term && minus_term ? expr_add(plus_term, minus_term) : NULL;
    expr_free(plus_term);
    expr_free(minus_term);
    if (!result)
        return NULL;

    entry = expr_div(result, eigenvalue_difference);
    expr_free(result);
                                          ...
                                          void *context)
{
    ...
    number_t four_determinant = number_invalid();
    number_t trace_squared = number_invalid();
    number_t discriminant = number_invalid();
    expr_t *discriminant_expr = NULL;
    expr_t *root = NULL;
    expr_t *trace_expr = NULL;
    expr_t *lambda_plus = NULL;
    expr_t *lambda_minus = NULL;
    expr_t *lambda_plus_power = NULL;
    expr_t *lambda_minus_power = NULL;
    matrix_t *result = NULL;

    if (!A || !exponent || A->rows != A->cols || (A->rows != 1u && A->rows != 2u) ||
    ...
    if (A->rows == 1u) {
        ...
        if (mat_typeof(A) == MAT_TYPE_EXPR) {
            mat_get(A, 0u, 0u, &base);
            if (base)
                expr_retain(base);
        } else {
            value = mat_get_num(A, 0u, 0u);
            base = expr_new_const(value);
            num_destroy(&value);
        }
    ...
    if (mat_typeof(A) == MAT_TYPE_EXPR) {
        ...
        expr_t *c = NULL;
        expr_t *d = NULL;
        expr_t *diagonal_difference = NULL;
        expr_t *diagonal_square = NULL;
        expr_t *off_diagonal_product = NULL;
        expr_t *four_off_diagonal_product = NULL;

        if (mat_trace_expr(A, &trace_expr) != 0)
            goto cleanup;
        mat_get(A, 0u, 0u, &a);
        mat_get(A, 0u, 1u, &b);
        mat_get(A, 1u, 0u, &c);
        mat_get(A, 1u, 1u, &d);
        diagonal_difference = a && d ? expr_sub(a, d) : NULL;
        diagonal_difference = expr_simplify_owned(diagonal_difference);
        diagonal_square = diagonal_difference ? expr_mul(diagonal_difference, diagonal_difference) : NULL;
        off_diagonal_product = b && c ? expr_mul(b, c) : NULL;
        four_off_diagonal_product = off_diagonal_product ? expr_mul_long(off_diagonal_product, 4L) : NULL;
        discriminant_expr = diagonal_square && four_off_diagonal_product
                                ? expr_add(diagonal_square, four_off_diagonal_product)
                                : NULL;
        expr_free(four_off_diagonal_product);
        expr_free(off_diagonal_product);
        expr_free(diagonal_square);
        expr_free(diagonal_difference);
        discriminant_expr = expr_simplify_owned(discriminant_expr);
    } else {
        if (mat_trace(A, &trace) != 0 || mat_det(A, &determinant) != 0)
            goto cleanup;
        trace_squared = num_mul(trace, trace);
        four_determinant = num_mul(four, determinant);
        discriminant = num_sub(trace_squared, four_determinant);
        if (!num_is_real(discriminant) || num_sign(discriminant) <= 0)
            goto cleanup;
        discriminant_expr = expr_new_const(discriminant);
        trace_expr = expr_new_const(trace);
    }
    root = discriminant_expr ? expr_sqrt(discriminant_expr) : NULL;
    if (root && trace_expr) {
        expr_t *sum = expr_add(trace_expr, root);
        expr_t *difference = expr_sub(trace_expr, root);

        lambda_plus = sum ? expr_div_long(sum, 2L) : NULL;
        lambda_minus = difference ? expr_div_long(difference, 2L) : NULL;
        expr_free(sum);
        expr_free(difference);
        lambda_plus = expr_simplify_owned(lambda_plus);
        lambda_minus = expr_simplify_owned(lambda_minus);
    }
...
cleanup:
    ...
    expr_free(lambda_plus);
    expr_free(trace_expr);
    expr_free(root);
    expr_free(discriminant_expr);
    num_destroy(&discriminant);
    num_destroy(&trace_squared);
    num_destroy(&four_determinant);
    num_destroy(&four);
...
/* ============================================================
   mat_pow  —  principal A^s for a real or complex number_t exponent
   ============================================================ */

/* Raise a numeric square matrix to a real or complex number_t power. */
matrix_t *mat_pow(const matrix_t *A, const number_t *s)
{
    ...
    result = mat_pow_diagonalizable(A, s);
    if (result)
        return result;
