...
/**
 * @brief the datetime type - this is the internal structure definition for the datetime type. It is not intended to be
 * used directly by users of the datetime library, but it is needed for the implementation of the datetime functions and
 ...
 * The algorithm used is the ELP2000-85 algorithm, which is a well-known method for calculating the time of the true new
 * moon. The algorithm takes the lunation number lunationIndex as input and calculates the time of the true new moon
 * based on a series of mathematical operations. The resulting time is returned as a Julian Ephemeris Date (JDE) in
 * days.
 * @param lunationIndex the lunation number for which to calculate the time of the true new moon. This should be a
 * positive integer.
 ...
 *
 * The difference between TT and UT is needed to convert between the two time standards. TT is a modern continuation of
 * Ephemeris Time (ET), and is the time standard used in astronomy. UT is the time standard used in everyday life. The
 * difference between TT and UT is caused by the Earth's slightly irregular rotation, and is usually expressed in
 * seconds.
 *
 * The algorithm used to calculate the difference is based on the IAU SOFA series, which provides a set of polynomial
 * expressions to calculate the difference between TT and UT for a given year. The expressions are based on a fit of
 * historical astronomical data, and are accurate to within 0.1 seconds for years between 1800 and 2500.
 *
 * @param year the year for which to calculate the difference between TT and UT. This should be a positive integer
 * between 1800 and 2500, inclusive. If the input year is outside this range, the function will return an approximate
 * value based on the nearest valid year.
 ...
 * This function takes a year between 1700 and 2400 and returns the Julian day number of the Chinese New Year in that
 * year.
...
static long datetime_india_new_moon_jdn_in_window(int year, month_t startMonth, uint8_t startDay, month_t endMonth,
                                                  uint8_t endDay);
static long datetime_local_new_moon_jdn_in_window(int year, month_t startMonth, uint8_t startDay, month_t endMonth,
...
static long datetime_next_local_new_moon_jdn(long afterJdn, int year, double gmtOffsetHours)
{
...
/**
 * @brief Converts a Julian Day Number into a Gregorian/Julian calendar date. Applies the Gregorian reform for JD >=
 * 2299161 and adjusts for the absence of a year zero (1 BC = year -1).
 ...
 *        This function calculates the sunrise or sunset time for the date represented by the datetime object and a
 * given location, and sets the time components of the datetime object accordingly. The location is specified by the
 * latitude and longitude. The time zone offset is used to adjust the calculated time to the local time zone (inclusive
 * of daylight saving time). If the calculated time is less than 0, it means that the sunrise/sunset occurs on the
 * previous day, so we add 24 hours to the time and subtract one day from the datetime object. If the calculated time is
 * greater than or equal to 24, it means that the sunrise/sunset occurs on the next day, so we subtract 24 hours from
 * the time and add one day to the datetime object.
 * @param dttm the datetime object to set the time components of. The date components of the datetime object should
 * already be set to the desired date.
 ...
 * @param timeZoneOffset the time zone offset in hours to adjust the calculated time to the local time zone (inclusive
 * of daylight saving time). For example, if the local time zone is GMT+2, the timeZoneOffset should be 2.0. If the
 * local time zone is GMT-5, the timeZoneOffset should be -5.0.
...
/**
 * @brief initialise a datetime object with the sunrise or sunset time for a given date and location. This function
 * calculates the sunrise or sunset time for a given date and location, and sets the time components of the datetime
 * object accordingly. The date is specified by the Julian Day Number, and the location is specified by the latitude and
 * longitude. The time zone offset is also taken into account to adjust the time to the local time zone (inclusive of
 * daylight saving time). If the calculated time is less than 0, it means that the sunrise/sunset occurs on the previous
 * day, so we add 24 hours to the time and subtract one day from the datetime object. If the calculated time is greater
 * than or equal to 24, it means that the sunrise/sunset occurs on the next day, so we subtract 24 hours from the time
 * and add one day to the datetime object. Finally, we update the hour, minute, and second components of the datetime
 * object based on the calculated time.
 ...
 * @param timeZoneOffset the time zone offset in hours to adjust the calculated time to the local time zone (inclusive
 * of daylight saving time). For example, if the local time zone is GMT+2, the timeZoneOffset should be 2.0. If the
 * local time zone is GMT-5, the timeZoneOffset should be -5.0. if the timeZoneOffset is set to DBL_MAX, the function
 * will attempt to calculate the time zone offset based on the datetime's date and the system's time zone information.
 * Note that this may not always be accurate, especially if the datetime's date is far in the past or future, or if the
 * system's time zone information is not up to date. Therefore, it is recommended to provide an explicit timeZoneOffset
 * whenever possible to ensure accurate results.
 * @param isSunrise a boolean value indicating whether to calculate the sunrise time (true) or the sunset time (false).
 * @return a pointer to the initialised datetime object. If the sunrise/sunset time cannot be calculated for the given
 * date and location (e.g. polar night), the time components of the datetime object will be set to 0, and the function
 * will return the datetime object with the date set to the given Julian Day Number.
 */
...
static datetime_t *datetime_init_sun_time_checked(datetime_t *dttm, long julianDayNumber, double latitude,
                                                  double longitude, double timeZoneOffset, bool isSunrise,
                                                  datetime_sun_status_t *status)
{
    double raw_time;
...
static datetime_t *datetime_init_adjacent_sun_time_checked(datetime_t *dttm, long julianDayNumber, double latitude,
                                                           double longitude, double timeZoneOffset, bool isSunrise,
                                                           int direction, datetime_sun_status_t *status)
{
...
/**
 * @brief calculate the moon phase for a given Julian Day Number. The moon phase is calculated based on the difference
 * between the given Julian Day Number and a known new moon date, divided by the length of a synodic month (the average
 * time between new moons). The result is then multiplied by 8 and
 * @param julianDayNumber the Julian Day Number to calculate the moon phase for. The Julian Day Number is a continuous
 * count of days since the beginning of the Julian Period, which is used in astronomy and other fields to represent
 * dates. It is calculated based on the date and time, and can be used to determine the position of celestial bodies,
 * including the moon phase.
