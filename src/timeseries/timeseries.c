...
char *ts_forecast_to_string(const ts_forecast_t *forecast, ts_string_style_t style)
{
    ...
    for (i = 0u; i < ts_length(forecast->mean); ++i) {
        ...
        if (style == TS_STRING_CSV) {
            ts_appendf(&sb, "%s,", date_text ? date_text : "");
            ts_append_number_text(&sb, value);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, stderr_v);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, lower_v);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, upper_v);
            ts_appendf(&sb, "\n");
        } else {
...
char *ts_regression_summary_to_string(const ts_regression_result_t *result)
{
    ...
    for (i = 0u; i < mat_get_row_count(result->coefficients); ++i) {
        ...
        ts_append_number_text(&sb, beta);
        ts_appendf(&sb, "  stderr = ");
        ts_append_number_text(&sb, se);
        ts_appendf(&sb, "\n");
        num_destroy(&beta);
        if (result->stderr)
            num_destroy(&se);
...
char *ts_arima_summary_to_string(const ts_arima_result_t *result)
{
    ...
    ts_append_number_text(&sb, result->summary.sigma2);
                                     ...
                                     ts_string_style_t style)
{
    ...
    if (style == TS_STRING_CSV && result && result->coefficients) {
        ...
        for (i = 0u; i < mat_get_row_count(result->coefficients); ++i) {
            ...
            ts_appendf(&sb, "beta%zu,", i);
            ts_append_number_text(&sb, est);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, se);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, t);
            ts_appendf(&sb, ",");
            ts_append_number_text(&sb, p);
            ts_appendf(&sb, "\n");
            num_destroy(&est);
            num_destroy(&se);
            num_destroy(&t);
...
{
    timeseries_t *dy = NULL, *dy_clean = NULL;
    matrix_t *lag_x = NULL, *x_trim = NULL, *X = NULL;
    ts_regression_result_t reg = {0};
    size_t p = 0u, rows, base_cols = 0u, i, r, c;
    bool include_intercept;
    ...
    if (p > 0u)
        lag_x = ts_design_matrix_lags(dy_clean, p, false);
    if (xreg) {
        size_t trim = p;
        size_t x_rows = mat_get_row_count(xreg);
        if (trim >= x_rows)
            goto fail;
        x_trim = ts_submatrix_rows(xreg, trim, x_rows - trim);
    }
    rows = dy_clean->length > p ? dy_clean->length - p : dy_clean->length;
    if (lag_x || x_trim) {
        base_cols = (lag_x ? mat_get_col_count(lag_x) : 0u) + (x_trim ? mat_get_col_count(x_trim) : 0u);
        X = mat_new(rows, base_cols);
        if (!X)
            goto fail;
        for (r = 0u; r < rows; ++r) {
            c = 0u;
            if (lag_x) {
                for (i = 0u; i < mat_get_col_count(lag_x); ++i) {
                    number_t v = mat_get_num(lag_x, r, i);
                    mat_set(X, r, c++, &v);
                    num_destroy(&v);
                }
            }
            if (x_trim) {
                for (i = 0u; i < mat_get_col_count(x_trim); ++i) {
                    number_t v = mat_get_num(x_trim, r, i);
                    mat_set(X, r, c++, &v);
                    num_destroy(&v);
                }
            }
    ...
    {
        timeseries_t *y_trim = p > 0u ? ts_slice(dy_clean, p, rows) : ts_clone(dy_clean);
        int rc = ts_regression_fit_internal(y_trim, X, include_intercept, spec->include_drift,
                                            options, &reg);
        ts_free(y_trim);
        if (rc != 0)
            goto fail;
    ...
    if (spec->p > 0u) {
        out->ar_params = mat_new(spec->p, 1u);
        if (!out->ar_params)
            goto fail;
        for (i = 0u; i < spec->p; ++i) {
            size_t offset = include_intercept ? 1u : 0u;
            number_t v = mat_get_num(reg.coefficients, offset + i, 0u);
            mat_set(out->ar_params, i, 0u, &v);
            num_destroy(&v);
        }
    }
    out->xreg_params = reg.coefficients;
    reg.coefficients = NULL;
    out->param_stderr = reg.stderr;
    reg.stderr = NULL;
    out->param_t_stat = reg.t_stat;
    reg.t_stat = NULL;
    out->param_p_value = reg.p_value;
    reg.p_value = NULL;
    out->vcov = reg.vcov;
    reg.vcov = NULL;
    out->fitted = reg.fitted;
    reg.fitted = NULL;
    out->residuals = reg.residuals;
    reg.residuals = NULL;
    out->innovations = ts_clone(out->residuals);
    out->summary = reg.summary;
    memset(&reg.summary, 0, sizeof(reg.summary));
    {
        ...
        if (ts_arima_meta_store(out, spec, include_intercept, intercept_ptr) != 0) {
            if (intercept_ptr)
    ...
    mat_free(lag_x); mat_free(x_trim); mat_free(X);
    return 0;
                      ...
                      ts_forecast_t *out)
{
    ...
    if (ts_series_to_double_array(history, &yd, &n, 0u) != 0 || n == 0u)
        goto fail;
    p = model->ar_params ? mat_get_row_count(model->ar_params) : 0u;
    start = ts_end_datetime(history);
    if (!start)
        goto fail;
    ...
    for (i = 0u; i < horizon; ++i) {
        double pred = 0.0;
        size_t j;
        number_t pred_n, se_n, width, lo, hi;

        if (meta && meta->has_intercept)
            pred += num_to_double(meta->intercept);
        for (j = 0u; j < p; ++j) {
            number_t phi = mat_get_num(model->ar_params, j, 0);
            double lagv = (n + i > j) ? (i > j ? num_to_double(mean->values[i - j - 1u]) : yd[n - j - 1u]) : 0.0;
            pred += num_to_double(phi) * lagv;
            num_destroy(&phi);
        }
        if (future_xreg && model->xreg_params) {
            size_t xcols = mat_get_col_count(future_xreg);
            size_t offset = (meta && meta->has_intercept ? 1u : 0u) + (meta && meta->spec.include_drift ? 1u : 0u) + p;

            for (j = 0u; j < xcols; ++j) {
                number_t beta = mat_get_num(model->xreg_params, offset + j, 0);
                number_t xv = mat_get_num(future_xreg, i, j);
                pred += num_to_double(beta) * num_to_double(xv);
                num_destroy(&beta);
                num_destroy(&xv);
            }
        }
        pred_n = num_create_from_double(pred);
        se_n = num_create_from_double(sigma * sqrt((double)(i + 1u)));
        width = num_mul(z, se_n);
        lo = num_sub(pred_n, width);
        hi = num_add(pred_n, width);
