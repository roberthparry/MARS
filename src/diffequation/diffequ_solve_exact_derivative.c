...
static expr_t *de_exact_formal_derivative(const expr_t *dependent,
...
static bool de_exact_contains_formal_derivative(const expr_t *expr)
{
    ...
    if (!expr_child_exprs(expr, &left, &right))
        return false;
    return de_exact_contains_formal_derivative(left) ||
           de_exact_contains_formal_derivative(right);
}
...
static expr_t *de_exact_integrated_right(const expr_t *forcing,
                                         const expr_t *independent)
{
    ...
    right = negative && constant
        ? expr_add_simplify_owned(negative, constant)
        : NULL;
    if (right) {
        negative = NULL;
        constant = NULL;
    ...
    size_t *solution_count_out)
{
    ...
    if (!de || !independent || !dependent || !first_derivative ||
        !second_derivative || !residual || !solutions_out ||
        !solution_count_out || de->condition_count != 0u)
        return DE_ATTEMPT_NOT_MATCHED;
    *solution_count_out = 0u;

    third_derivative =
        de_exact_formal_derivative(dependent, independent, 3u);
    if (!third_derivative ||
        !de_linear_decompose(
            residual, third_derivative, &leading, &after_third) ||
        !leading || !de_exact_is_numeric_constant(leading) ||
        ...
        !de_linear_decompose(
            ...
            &mixed_scale,
            &mixed_offset) ||
        !mixed_scale || !de_exact_is_numeric_constant(mixed_scale) ||
        expr_is_exact_zero(mixed_scale) ||
        !mixed_offset || !expr_is_exact_zero(mixed_offset) ||
    ...
    integrated_right =
        de_exact_integrated_right(forcing, independent);
    auxiliary = expr_new_named_var(NUM_NAN, "u");
    auxiliary_second =
        de_exact_formal_derivative(auxiliary, independent, 2u);
    absolute_auxiliary = auxiliary ? expr_abs(auxiliary) : NULL;
    log_auxiliary =
        absolute_auxiliary ? expr_log(absolute_auxiliary) : NULL;
    dependent_scale = expr_div_simplify_owned(
        expr_mul_long(leading, 2L),
        expr_clone(mixed_scale));
    dependent_right = dependent_scale && log_auxiliary
        ? expr_mul_simplify_owned(dependent_scale, log_auxiliary)
        : NULL;
    if (dependent_right) {
    ...
    linear_scale_denominator =
        leading_square ? expr_mul_long(leading_square, 2L) : NULL;
    linear_scale = linear_scale_denominator
    ...
    if (!cubic_term ||
        !de_linear_decompose(
            ...
            &cubic_remainder))
        goto cleanup;
    scaled_cubic_coefficient = linear_scale && cubic_coefficient
        ? expr_mul_simplify_owned(
              expr_new_const(NUM_TWO),
              expr_mul_simplify_owned(
                  expr_clone(linear_scale),
                  expr_clone(cubic_coefficient)))
        : NULL;
    canonical_parameter = linear_scale && cubic_remainder
        ? expr_mul_simplify_owned(
              expr_new_const(NUM_TWO),
              expr_mul_simplify_owned(
                  expr_clone(linear_scale),
                  expr_clone(cubic_remainder)))
        : NULL;
    linear_right = linear_scale && integrated_right && auxiliary
        ? expr_mul_simplify_owned(
              expr_mul_simplify_owned(
                  linear_scale,
                  expr_clone(integrated_right)),
              expr_clone(auxiliary))
        : NULL;
    if (linear_right)
        linear_scale = NULL;

    if (!integrated_right || !auxiliary || !auxiliary_second ||
        !dependent_right || !linear_right || !canonical_parameter ||
        !scaled_cubic_coefficient ||
        !de_exact_is_numeric_one(scaled_cubic_coefficient) ||
     ...
     */
    if (de_sturm_liouville_cubic_basis(
            independent,
            canonical_parameter,
            auxiliary,
            solutions_out,
        ...
        goto cleanup;
    transformed_solution = expr_substitute(
        dependent_right,
        auxiliary,
        equ_rhs(solutions_out[0]));
    dependent_solution = transformed_solution
        ? equ_new(dependent, transformed_solution)
        : NULL;
