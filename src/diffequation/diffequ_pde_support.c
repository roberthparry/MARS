    ...
    const expr_t **dy_out)
{
    ...
    if (!expr_child_exprs(expr, &left, &right))
        return true;
    return de_pde_find_first_derivatives(
               left, x, y, dependent_out, dx_out, dy_out) &&
           de_pde_find_first_derivatives(
    ...
    const char *name)
{
    ...
    if (!expr_child_exprs(expr, &left, &right))
        return NULL;
    found = de_pde_find_named_coordinate(
        left, independent, dependent, name);
    return found
        ? found
        : de_pde_find_named_coordinate(
              right, independent, dependent, name);
}
