...
static expr_t *de_derivative_quadratic_independent_symbol(const diffequ_t *de)
{
    const char *text;
    const char *equals;
    const char *end;
...
static expr_t *de_derivative_quadratic_coefficient(const expr_t *coefficient, const expr_t *dependent_coefficient)
{
    expr_t *quotient = expr_div_simplify_owned(expr_clone(coefficient), expr_clone(dependent_coefficient));

    return de_simplify_unary_owned(quotient, expr_neg);
}

static expr_t *de_derivative_quadratic_root(const expr_t *quadratic, const expr_t *linear, const expr_t *constant,
                                            bool positive)
{
    ...
    expr_t *numerator =
        positive ? expr_add_simplify_owned(negative_linear, root) : expr_sub_simplify_owned(negative_linear, root);
...
static expr_t *de_derivative_quadratic_implicit_rhs(const expr_t *quadratic, const expr_t *linear, const expr_t *root)
{
    expr_t *linear_root = expr_mul_simplify_owned(expr_mul_long(quadratic, 2L), expr_clone(root));
    expr_t *absolute = expr_abs(root);
    expr_t *logarithm = absolute ? expr_log(absolute) : NULL;
    expr_t *log_term = logarithm ? expr_mul_simplify_owned(expr_clone(linear), logarithm) : NULL;
    expr_t *rhs = NULL;
...
de_attempt_t de_attempt_derivative_quadratic(const diffequ_t *de, const expr_t *independent, const expr_t *dependent,
                                             ...
                                             equation_t **solutions_out, size_t *solution_count_out)
{
    ...
    *solution_count_out = 0u;
    ...
    if (!equ_match_symbolic_quadratic_expr(residual, first_derivative, &constant, &linear, &quadratic) || !quadratic ||
        expr_is_exact_zero(quadratic) || de_expr_uses(quadratic, independent) || de_expr_uses(quadratic, dependent) ||
        de_expr_uses(linear, independent) || de_expr_uses(linear, dependent) ||
        !de_linear_decompose(constant, dependent, &dependent_coefficient, &offset) ||
        expr_is_exact_zero(dependent_coefficient) || de_expr_uses(dependent_coefficient, independent) ||
        de_expr_uses(dependent_coefficient, dependent) || de_expr_uses(offset, independent) ||
        de_expr_uses(offset, dependent))
        goto cleanup;

    attempt = DE_ATTEMPT_FAILED;
    a = de_derivative_quadratic_coefficient(quadratic, dependent_coefficient);
    b = de_derivative_quadratic_coefficient(linear, dependent_coefficient);
    d = de_derivative_quadratic_coefficient(offset, dependent_coefficient);
    if (!a || !b || !d || expr_is_exact_zero(a))
        goto cleanup;

    positive_root = de_derivative_quadratic_root(quadratic, linear, constant, true);
    negative_root = de_derivative_quadratic_root(quadratic, linear, constant, false);
    positive_rhs = positive_root ? de_derivative_quadratic_implicit_rhs(a, b, positive_root) : NULL;
    negative_rhs = negative_root ? de_derivative_quadratic_implicit_rhs(a, b, negative_root) : NULL;
    if (!positive_rhs || !negative_rhs)
        goto cleanup;

    independent_symbol = de_derivative_quadratic_independent_symbol(de);
    ...
    if (!solutions_out[0] || !solutions_out[1] || !solutions_out[2]) {
