...
static expr_t *de_bernoulli_constant(
    const diffequ_t *de,
    const expr_t *dependent,
    ...
    const expr_t *integral)
{
    ...
    scaled_value = reciprocal_value && factor_at_point
        ? expr_mul_simplify_owned(
              reciprocal_value, factor_at_point)
        : NULL;
...
{
    const expr_t *dependent_square =
        de_find_dependent_square(derivative_right, dependent);
    ...
    if (!dependent_square ||
        ...
        !de_linear_decompose(
            ...
            &constant_term))
        goto cleanup;
    if (!expr_is_exact_zero(constant_term) ||
        de_expr_uses(square_coefficient, dependent) ||
    ...
    coefficient_integral =
        de_integrate_or_formal(linear_coefficient, independent);
    integrating_factor = coefficient_integral
        ? expr_exp(coefficient_integral)
        : NULL;
    negative_square_coefficient =
        de_simplify_unary_owned(square_coefficient, expr_neg);
    square_coefficient = NULL;
    weighted_forcing = integrating_factor
        ? expr_mul_simplify_owned(
              expr_clone(integrating_factor),
              negative_square_coefficient)
        : NULL;
    ...
    constant = de_bernoulli_constant(
        de,
        dependent,
        independent,
        coefficient_integral,
    ...
    transformed_solution = transformed_solution
        ? expr_div_simplify_owned(
              transformed_solution, integrating_factor)
        : NULL;
    integrating_factor = NULL;
    solved_right = transformed_solution
        ? expr_div_simplify_owned(
              expr_const_one(), transformed_solution)
        : NULL;
