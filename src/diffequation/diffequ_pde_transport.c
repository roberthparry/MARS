...
static bool de_transport_is_constant_term(
    const expr_t *term,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent)
...
static expr_t *de_transport_additive_forcing_solution(
    const expr_t *first,
    const expr_t *second,
    ...
    for (size_t i = 0u; i < de->condition_count; ++i) {
        ...
        travel = foot
            ? expr_sub_simplify_owned(
                  expr_clone(along), expr_clone(foot))
            : NULL;
        solution_right = NULL;
        if (substituted && travel &&
            expr_is_exact_zero(reaction_coefficient)) {
            expr_t *zero = expr_const_zero();

            particular = zero
                ? de_transport_additive_forcing_solution(
                      x,
                      y,
                      x_coefficient,
                      y_coefficient,
                ...
                : NULL;
            zero = NULL;
            particular_at_transverse = particular
                ? expr_substitute(
                      particular, transverse, fixed_transverse)
                : NULL;
            particular_at_boundary = particular_at_transverse
                ? expr_substitute(
                      particular_at_transverse, along, foot)
                : NULL;
            if (particular && particular_at_boundary) {
                expr_t *with_boundary_value =
                    expr_add(particular, substituted);

                solution_right = with_boundary_value
                    ? expr_sub(
                          with_boundary_value,
            ...
            expr_t *offset =
                expr_div(forcing, reaction_coefficient);
            expr_t *adjusted = offset
                ? expr_add(substituted, offset)
                : NULL;
            expr_t *alpha = expr_div(
                reaction_coefficient, along_coefficient);
            expr_t *exponent = alpha
                ? expr_mul(alpha, travel)
                : NULL;
            expr_t *negative_exponent =
                exponent ? expr_neg(exponent) : NULL;
            expr_t *decay =
                negative_exponent ? expr_exp(negative_exponent) : NULL;
            expr_t *evolved = adjusted && decay
                ? expr_mul(adjusted, decay)
                : NULL;

            solution_right = evolved && offset
                ? expr_sub(evolved, offset)
                : NULL;
            expr_free(evolved);
            expr_free(decay);
        ...
        solution_right = solution_right
            ? expr_simplify_owned(solution_right)
            : NULL;
        ...
        if (!solution_right)
            return NULL;
        solution =
            de_pde_solution_equation(dependent, solution_right);
...
static expr_t *de_transport_directional_derivative(
    const expr_t *expr,
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient)
{
    ...
    expr_t *first_term = first_derivative
        ? expr_mul(first_coefficient, first_derivative)
        : NULL;
    expr_t *second_term = second_derivative
        ? expr_mul(second_coefficient, second_derivative)
        : NULL;
    expr_t *raw = first_term && second_term
...
static expr_t *de_transport_closed_directional_particular(
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient,
    ...
    const expr_t *forcing)
{
    expr_t *first_directional = de_transport_directional_derivative(
        forcing,
        first,
        second,
        first_coefficient,
    ...
    expr_t *second_directional = first_directional
        ? de_transport_directional_derivative(
              first_directional,
              first,
              second,
              first_coefficient,
              second_coefficient)
        : NULL;
    expr_t *derivative_scale = second_directional
        ? expr_div_simplify_owned(
              expr_clone(second_directional), expr_clone(forcing))
        : NULL;
    expr_t *reconstructed = derivative_scale
        ? expr_mul_simplify_owned(
              expr_clone(derivative_scale), expr_clone(forcing))
        : NULL;
    expr_t *remainder = reconstructed
        ? expr_sub_simplify_owned(
              expr_clone(second_directional), reconstructed)
        : NULL;
    ...
    if (!second_directional ||
        ...
        de_expr_uses(derivative_scale, first) ||
        de_expr_uses(derivative_scale, second))
        goto cleanup;

    reaction_square = expr_mul(
        reaction_coefficient, reaction_coefficient);
    denominator = reaction_square
        ? expr_sub_simplify_owned(
              reaction_square, expr_clone(derivative_scale))
        : NULL;
    ...
    numerator = reaction_forcing
        ? expr_sub_simplify_owned(
              expr_clone(first_directional), reaction_forcing)
        : NULL;
    reaction_forcing = NULL;
    particular = numerator
        ? expr_div_simplify_owned(
              numerator, expr_clone(denominator))
        : NULL;
    numerator = NULL;
    particular_directional = particular
        ? de_transport_directional_derivative(
              particular,
              first,
              second,
              ...
              second_coefficient)
        : NULL;
    particular_reaction = particular
        ? expr_mul(reaction_coefficient, particular)
        : NULL;
    applied = particular_directional && particular_reaction
        ? expr_add_simplify_owned(
              particular_directional, particular_reaction)
        : NULL;
    particular_directional = NULL;
    particular_reaction = NULL;
    check = applied
        ? expr_add_simplify_owned(applied, expr_clone(forcing))
        : NULL;
    applied = NULL;
...
static expr_t *de_transport_unary_phase_particular(
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient,
    const expr_t *forcing)
{
    ...
    if (!expr_match_unary_expr(base, &phase))
        goto cleanup;
    phase_directional = de_transport_directional_derivative(
        phase,
        first,
        second,
        first_coefficient,
        second_coefficient);
    if (!phase_directional ||
        expr_is_exact_zero(phase_directional) ||
        de_expr_uses(phase_directional, first) ||
    ...
    dummy_primitive = dummy_base
        ? expr_integrate(dummy_base, dummy)
        : NULL;
    phase_primitive = dummy_primitive
        ? expr_substitute(dummy_primitive, dummy, phase)
        : NULL;
    if (phase_primitive &&
        expr_match_const_value(phase_directional, &phase_rate)) {
        number_t coefficient = num_div(scale, phase_rate);

        candidate = expr_mul_num(phase_primitive, &coefficient);
        num_destroy(&coefficient);
    } else {
        scaled_primitive = phase_primitive
            ? expr_mul_num(phase_primitive, &scale)
            : NULL;
        candidate = scaled_primitive
            ? expr_div_simplify_owned(
                  scaled_primitive, expr_clone(phase_directional))
            : NULL;
        scaled_primitive = NULL;
    }
    candidate_directional = candidate
        ? de_transport_directional_derivative(
              candidate,
              first,
              second,
              ...
              second_coefficient)
        : NULL;
    check = candidate_directional
        ? expr_add_simplify_owned(
              candidate_directional, expr_clone(forcing))
        : NULL;
...
static expr_t *de_transport_prepend_sum(
    const expr_t *prefix,
    const expr_t *tail);

static expr_t *de_transport_combine_particular(
    const expr_t *first,
    const expr_t *first_coefficient,
    const expr_t *reaction_coefficient,
    ...
    const expr_t *particular)
{
    ...
    if (expr_is_exact_zero(reaction_coefficient)) {
        ...
        alpha = expr_div_simplify_owned(
            expr_clone(reaction_coefficient),
            expr_clone(first_coefficient));
        exponent = alpha
            ? expr_mul_simplify_owned(alpha, expr_clone(first))
            : NULL;
        alpha = NULL;
    ...
    right = homogeneous && particular
        ? de_transport_prepend_sum(homogeneous, particular)
        : NULL;
...
static expr_t *de_transport_variable_forcing_solution(
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient,
    ...
    expr_t *arbitrary)
{
    ...
    expr_t *ratio = expr_div_simplify_owned(
        expr_clone(second_coefficient),
        expr_clone(first_coefficient));
    expr_t *path_shift = ratio
        ? expr_mul_simplify_owned(ratio, expr_clone(first))
        : NULL;
    expr_t *path_second = eta && path_shift
        ? expr_add_simplify_owned(expr_clone(eta), path_shift)
        : NULL;
    expr_t *forcing_path = path_second
        ? expr_substitute(forcing, second, path_second)
        : NULL;
    expr_t *negative_forcing = forcing_path
        ? expr_negate_owned(forcing_path)
        : NULL;
    expr_t *source = negative_forcing
        ? expr_div_simplify_owned(
              negative_forcing, expr_clone(first_coefficient))
        : NULL;
    expr_t *alpha = expr_div_simplify_owned(
        expr_clone(reaction_coefficient),
        expr_clone(first_coefficient));
    expr_t *alpha_first = alpha
        ? expr_mul_simplify_owned(alpha, expr_clone(first))
        : NULL;
    expr_t *factor =
        alpha_first ? expr_exp(alpha_first) : NULL;
    expr_t *weighted = source && factor
        ? expr_mul_simplify_owned(source, expr_clone(factor))
        : NULL;
    expr_t *antiderivative = weighted
        ? de_integrate_or_formal(weighted, first)
        : NULL;
    expr_t *current_antiderivative =
        antiderivative && eta
            ? expr_substitute(antiderivative, eta, invariant)
            : NULL;
    expr_t *particular = current_antiderivative && factor
        ? expr_div_simplify_owned(
              expr_clone(current_antiderivative),
              expr_clone(factor))
        : NULL;
    expr_t *homogeneous = arbitrary && factor
        ? expr_div_simplify_owned(
              expr_clone(arbitrary),
              expr_clone(factor))
        : NULL;
    expr_t *right = particular && homogeneous
        ? expr_add(particular, homogeneous)
        : NULL;
    ...
    expr_t *arbitrary)
{
    ...
    expr_t *at_second_zero = zero
        ? expr_substitute(forcing, second, zero)
        : NULL;
    expr_t *origin = at_second_zero && zero
        ? expr_substitute(at_second_zero, first, zero)
        : NULL;
    expr_t *first_part = at_second_zero && origin
        ? expr_sub_simplify_owned(
              expr_clone(at_second_zero), expr_clone(origin))
        : NULL;
    expr_t *second_part = zero
        ? expr_substitute(forcing, first, zero)
        : NULL;
    expr_t *separated = first_part && second_part
        ? expr_add_simplify_owned(
              expr_clone(first_part), expr_clone(second_part))
        : NULL;
    expr_t *mixed = separated
        ? expr_sub_simplify_owned(
              expr_clone(forcing), separated)
        : NULL;
    expr_t *first_source = first_part
        ? expr_div_simplify_owned(
              expr_negate_owned(expr_clone(first_part)),
              expr_clone(first_coefficient))
        : NULL;
    expr_t *second_source = second_part
        ? expr_div_simplify_owned(
              expr_negate_owned(expr_clone(second_part)),
              expr_clone(second_coefficient))
        : NULL;
    ...
    if (!mixed || !expr_is_exact_zero(mixed))
        goto cleanup;
    first_integral = first_source
        ? de_integrate_or_formal(first_source, first)
        : NULL;
    second_integral = second_source
        ? de_integrate_or_formal(second_source, second)
        : NULL;
    particular = first_integral && second_integral
        ? expr_add_simplify_owned(
              expr_clone(first_integral),
              expr_clone(second_integral))
        : NULL;
    right = particular && arbitrary
        ? expr_add_simplify_owned(
    ...
    const expr_t *tail)
{
    ...
    if (expr_match_add_sub_expr(
            tail, &left, &right, &is_sub)) {
        expr_t *combined_left =
            de_transport_prepend_sum(prefix, left);
        expr_t *combined = combined_left
            ? (is_sub
                ? expr_sub(combined_left, right)
                : expr_add(combined_left, right))
            : NULL;
...
static expr_t *de_transport_closed_derivative_particular(
    const expr_t *variable,
    const expr_t *coefficient,
    const expr_t *reaction_coefficient,
    const expr_t *forcing)
{
    expr_t *first_derivative =
        expr_create_deriv(forcing, variable);
    expr_t *second_derivative = first_derivative
        ? expr_create_deriv(first_derivative, variable)
        : NULL;
    expr_t *derivative_scale = second_derivative
        ? expr_div_simplify_owned(
              expr_clone(second_derivative),
              expr_clone(forcing))
        : NULL;
    expr_t *reconstructed = derivative_scale
        ? expr_mul_simplify_owned(
              expr_clone(derivative_scale),
              expr_clone(forcing))
        : NULL;
    expr_t *remainder = reconstructed
        ? expr_sub_simplify_owned(
    ...
    reaction_square = expr_mul(
        reaction_coefficient, reaction_coefficient);
    coefficient_square =
        expr_mul(coefficient, coefficient);
    scaled_coefficient_square =
        coefficient_square && derivative_scale
            ? expr_mul(
                  coefficient_square, derivative_scale)
            : NULL;
    denominator =
        reaction_square && scaled_coefficient_square
            ? expr_sub_simplify_owned(
                  reaction_square,
                  scaled_coefficient_square)
            : NULL;
    ...
    derivative_term = first_derivative
        ? expr_mul(coefficient, first_derivative)
        : NULL;
    forcing_term =
        expr_mul(reaction_coefficient, forcing);
    numerator = derivative_term && forcing_term
        ? expr_sub_simplify_owned(
              derivative_term, forcing_term)
        : NULL;
...
static expr_t *de_transport_coordinate_reaction_particular(
    const expr_t *variable,
    const expr_t *coefficient,
    const expr_t *reaction_coefficient,
    const expr_t *forcing)
{
    ...
    expr_t *closed =
        de_transport_closed_derivative_particular(
            variable,
            coefficient,
    ...
    expr_t *exponent = alpha
        ? expr_mul_simplify_owned(alpha, expr_clone(variable))
        : NULL;
    expr_t *factor = exponent ? expr_exp(exponent) : NULL;
    expr_t *source = expr_div_simplify_owned(
        expr_negate_owned(expr_clone(forcing)),
        expr_clone(coefficient));
    expr_t *weighted = source && factor
        ? expr_mul_simplify_owned(
              source, expr_clone(factor))
        : NULL;
    expr_t *antiderivative = weighted
        ? de_integrate_or_formal(weighted, variable)
        : NULL;
    expr_t *particular = antiderivative && factor
        ? expr_div_simplify_owned(
              expr_clone(antiderivative),
              expr_clone(factor))
        : NULL;
...
static expr_t *de_transport_additive_reaction_solution(
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient,
    ...
    expr_t *arbitrary)
{
    ...
    if (!mixed || !expr_is_exact_zero(mixed))
        goto cleanup;
    first_solution =
        de_transport_coordinate_reaction_particular(
            first,
            first_coefficient,
            reaction_coefficient,
            first_part);
    ...
    particular = first_solution && second_solution
        ? expr_add_simplify_owned(
              expr_clone(first_solution),
              expr_clone(second_solution))
        : NULL;
    ...
    negative_exponent = exponent
        ? expr_negate_owned(exponent)
        : NULL;
    exponent = NULL;
    decay = negative_exponent
        ? expr_exp(negative_exponent)
        : NULL;
    homogeneous = decay && arbitrary
        ? expr_mul(decay, arbitrary)
        : NULL;
    right = homogeneous && particular
        ? de_transport_prepend_sum(homogeneous, particular)
        : NULL;
...
static expr_t *de_transport_polynomial_reaction_solution(
    const expr_t *first,
    const expr_t *second,
    const expr_t *first_coefficient,
    const expr_t *second_coefficient,
    ...
    expr_t *arbitrary)
{
    expr_t *negative_forcing =
        expr_negate_owned(expr_clone(forcing));
    expr_t *current = negative_forcing
        ? expr_div_simplify_owned(
              negative_forcing,
              expr_clone(reaction_coefficient))
        : NULL;
    ...
    expr_t *exponent = alpha
        ? expr_mul_simplify_owned(alpha, expr_clone(first))
        : NULL;
    expr_t *negative_exponent = exponent
        ? expr_negate_owned(exponent)
        : NULL;
    expr_t *decay = negative_exponent
        ? expr_exp(negative_exponent)
        : NULL;
    expr_t *homogeneous = decay && arbitrary
        ? expr_mul(decay, arbitrary)
        : NULL;
    expr_t *right = NULL;
    ...
    for (size_t degree = 0u;
        ...
        if (expr_is_exact_zero(directional)) {
            right = homogeneous
                ? de_transport_prepend_sum(homogeneous, sum)
                : NULL;
        ...
        negative_directional =
            expr_negate_owned(directional);
        directional = NULL;
        next = negative_directional
            ? expr_div_simplify_owned(
                  negative_directional,
                  expr_clone(reaction_coefficient))
            : NULL;
    ...
    const expr_t *forcing)
{
    ...
    if (!arbitrary)
        return NULL;
    if (de_expr_uses(forcing, first) ||
        de_expr_uses(forcing, second)) {
        invariant = de_transport_invariant(
            first,
            second,
            first_coefficient,
            second_coefficient);
        if (invariant &&
            expr_is_exact_zero(reaction_coefficient))
            right = de_transport_additive_forcing_solution(
                first,
                second,
                first_coefficient,
                second_coefficient,
        ...
        if (!right &&
            invariant &&
            !expr_is_exact_zero(reaction_coefficient))
            right = de_transport_polynomial_reaction_solution(
                first,
                second,
                first_coefficient,
                second_coefficient,
        ...
        if (!right &&
            invariant &&
            !expr_is_exact_zero(reaction_coefficient))
            right = de_transport_additive_reaction_solution(
                first,
                second,
                first_coefficient,
                second_coefficient,
                ...
                expr_clone(arbitrary));
        if (!right && invariant)
            particular = de_transport_closed_directional_particular(
                first,
                second,
                first_coefficient,
                second_coefficient,
                reaction_coefficient,
                forcing);
        if (!right && !particular && invariant &&
            expr_is_exact_zero(reaction_coefficient))
            particular = de_transport_unary_phase_particular(
                first,
                second,
                first_coefficient,
                second_coefficient,
                forcing);
        if (!right && particular)
            right = de_transport_combine_particular(
                first,
                first_coefficient,
                reaction_coefficient,
                arbitrary,
                particular);
        if (!right && invariant)
            right = de_transport_variable_forcing_solution(
                  first,
                  second,
                  first_coefficient,
                  second_coefficient,
        ...
        expr_t *exponent = alpha
            ...
            : NULL;
        expr_t *negative_exponent =
            exponent ? expr_negate_owned(exponent) : NULL;
        expr_t *decay =
            negative_exponent ? expr_exp(negative_exponent) : NULL;
        expr_t *homogeneous = decay
            ? expr_mul_simplify_owned(decay, arbitrary)
            : NULL;
        expr_t *offset = expr_div_simplify_owned(
            expr_clone(forcing),
            expr_clone(reaction_coefficient));
        ...
        if (homogeneous)
            decay = NULL;
        right = homogeneous && offset
            ? expr_sub_simplify_owned(homogeneous, offset)
            : NULL;
        if (right) {
            homogeneous = NULL;
            offset = NULL;
    ...
    bool *recognized_out)
{
    ...
    *recognized_out = false;
    ...
    if (!de_pde_find_first_derivatives(
        ...
        !de_transport_is_constant_term(
            reaction_coefficient, x, y, dependent) ||
    ...
    *recognized_out = true;
    if (de->condition_count > 0u) {
        if (expr_is_exact_zero(reaction_coefficient) ||
            (!de_expr_uses(forcing, x) &&
             !de_expr_uses(forcing, y)))
...
static bool de_transport_is_constant_in_all(
    const expr_t *expr,
    const diffequ_t *de,
    const expr_t *dependent)
{
...
static expr_t *de_transport_multi_invariant(
    const expr_t *anchor,
    const expr_t *variable,
    const expr_t *anchor_coefficient,
    const expr_t *coefficient)
{
    expr_t *ratio = expr_div_simplify_owned(
        expr_clone(coefficient),
        expr_clone(anchor_coefficient));
    expr_t *shift = ratio
        ? expr_mul_simplify_owned(ratio, expr_clone(anchor))
        : NULL;

    ratio = NULL;
    if (!shift)
        return NULL;
    return expr_sub_simplify_owned(
        expr_clone(variable), shift);
}

static expr_t *de_transport_multi_right(
    const diffequ_t *de,
    ...
    const expr_t *forcing)
{
    ...
    expr_t **invariants =
        calloc(invariant_count, sizeof(*invariants));
    ...
    for (size_t i = 0u; i < invariant_count; ++i) {
        invariants[i] = de_transport_multi_invariant(
            de->independent_vars[0],
            de->independent_vars[i + 1u],
            coefficients[0],
            coefficients[i + 1u]);
        if (!invariants[i])
            goto cleanup;
    }
    arbitrary = expr_new_arbitrary_function_n(
        "F", invariant_count, invariants);
    if (!arbitrary)
        goto cleanup;
    ...
    if (expr_is_exact_zero(reaction_coefficient)) {
        if (expr_is_exact_zero(forcing)) {
            ...
            expr_t *rate = expr_div_simplify_owned(
                expr_negate_owned(expr_clone(forcing)),
                expr_clone(coefficients[0]));
            expr_t *particular = rate
                ? expr_mul_simplify_owned(
                      rate,
                      expr_clone(de->independent_vars[0]))
                : NULL;

            rate = NULL;
            right = particular
                ? expr_add_simplify_owned(arbitrary, particular)
                : NULL;
            if (right) {
                arbitrary = NULL;
                particular = NULL;
    ...
    bool *recognized_out)
{
    ...
    *solution_out = NULL;
    *recognized_out = false;
    if (!de || !residual || de->independent_count < 3u ||
        de->condition_count != 0u)
        return DE_ATTEMPT_NOT_MATCHED;

    derivatives = calloc(
        de->independent_count, sizeof(*derivatives));
    coefficients = calloc(
        de->independent_count, sizeof(*coefficients));
    if (!derivatives || !coefficients) {
        attempt = DE_ATTEMPT_FAILED;
        goto cleanup;
    }
    if (!de_pde_find_first_derivatives_n(
            residual,
            de->independent_count,
            de->independent_vars,
            &dependent,
            derivatives) ||
        !dependent)
    ...
    if (!de_linear_decompose(
        ...
        goto cleanup;
    for (size_t i = 0u; i < de->independent_count; ++i) {
        if (expr_is_exact_zero(coefficients[i]) ||
            !de_transport_is_constant_in_all(
                coefficients[i], de, dependent))
            goto cleanup;
    }
    if (!de_transport_is_constant_in_all(
            reaction_coefficient, de, dependent) ||
        !de_transport_is_constant_in_all(forcing, de, dependent))
        goto cleanup;

    *recognized_out = true;
    right = de_transport_multi_right(
        de, coefficients, reaction_coefficient, forcing);
    if (!right) {
        attempt = DE_ATTEMPT_FAILED;
        goto cleanup;
    }
