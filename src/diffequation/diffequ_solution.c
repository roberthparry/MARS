    ...
    diffequ_solve_result_t *result)
{
    ...
    if (!result || result->status != DE_SOLVE_STATUS_SOLVED ||
    ...
    if (!problem || !problem_tex || !steps || !steps_tex ||
        string_append_format(
            ...
            "Recognise the %s.\nParsed equation:\n      %s\nRule:\n"
            "      %s\nDerived solution%s:\n",
            de_solver_rule_name(result->solver),
            problem,
            de_solver_rule_plain(result->solver),
        ...
        string_append_format(
            ...
            de_solver_rule_name(result->solver),
            problem_tex,
            de_solver_rule_tex(result->solver)) < 0)
        goto cleanup;
