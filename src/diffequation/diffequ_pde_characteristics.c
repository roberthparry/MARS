...
static equation_t *de_pde_quadratic_boundary_solution(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *invariant,
    const expr_t *scaled_potential);

static equation_t *de_pde_dependent_square_boundary_solution(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
...
static bool de_pde_is_characteristic_constant(
    const expr_t *expr,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent)
...
static expr_t *de_pde_monomial_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
...
{
    expr_t *numerator = expr_mul_simplify_owned(
        expr_clone(y_coefficient), expr_clone(x));
    expr_t *denominator = expr_mul_simplify_owned(
        expr_clone(x_coefficient), expr_clone(y));
    expr_t *power_ratio = numerator && denominator
    ...
    numerator = NULL;
    denominator = NULL;
    if (!de_pde_is_characteristic_constant(
            power_ratio, x, y, dependent))
        goto cleanup;
    negative_power = expr_negate_owned(power_ratio);
    power_ratio = NULL;
    if (negative_power &&
        de_pde_exact_long(negative_power, &integer_power) &&
        integer_power == -1L) {
        invariant = expr_div_simplify_owned(
            expr_clone(y), expr_clone(x));
    } else {
        x_power = negative_power
            ? (de_pde_exact_long(negative_power, &integer_power)
                  ? expr_pow_long(x, integer_power)
                  : expr_pow_xp(x, negative_power))
            : NULL;
        invariant = x_power
            ? expr_mul_simplify_owned(expr_clone(y), x_power)
            : NULL;
        x_power = NULL;
    }
...
static expr_t *de_pde_radial_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient)
{
    ...
    expr_t *invariant = x_squared && y_squared
    ...
    along_x = invariant_x
        ? expr_mul(x_coefficient, invariant_x)
        : NULL;
    along_y = invariant_y
        ? expr_mul(y_coefficient, invariant_y)
        : NULL;
    check_raw = along_x && along_y
        ? expr_add(along_x, along_y)
        : NULL;
...
static expr_t *de_pde_scaled_coordinate_invariant_candidate(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
...
{
    expr_t *candidate = expr_div_simplify_owned(
        expr_clone(numerator), expr_clone(denominator));
    expr_t *candidate_x = candidate
        ? expr_create_deriv(candidate, x)
        : NULL;
    expr_t *candidate_y = candidate
        ? expr_create_deriv(candidate, y)
        : NULL;
    expr_t *along_x = candidate_x
        ? expr_mul(x_coefficient, candidate_x)
        : NULL;
    expr_t *along_y = candidate_y
        ? expr_mul(y_coefficient, candidate_y)
        : NULL;
    expr_t *check_raw = along_x && along_y
        ? expr_add(along_x, along_y)
        : NULL;
...
static expr_t *de_pde_scaled_coordinate_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient)
{
    expr_t *derivative = !de_expr_uses(x_coefficient, y)
        ? expr_create_deriv(x_coefficient, x)
        : NULL;
    expr_t *expected = derivative
        ? expr_mul_simplify_owned(
              expr_clone(y), expr_clone(derivative))
        : NULL;
    expr_t *invariant = expected && de_pde_same_symbolic_form(
            expected, y_coefficient)
        ? expr_div_simplify_owned(
              expr_clone(y), expr_clone(x_coefficient))
        : NULL;
    ...
    derivative = !de_expr_uses(y_coefficient, x)
        ? expr_create_deriv(y_coefficient, y)
        : NULL;
    expected = derivative
        ? expr_mul_simplify_owned(
              expr_clone(x), expr_clone(derivative))
        : NULL;
    invariant = expected && de_pde_same_symbolic_form(
            expected, x_coefficient)
        ? expr_div_simplify_owned(
              expr_clone(x), expr_clone(y_coefficient))
        : NULL;
    expr_free(expected);
    expr_free(derivative);
    if (invariant)
    ...
    invariant = de_pde_scaled_coordinate_invariant_candidate(
            x,
            y,
            x_coefficient,
            y_coefficient,
...
static bool de_pde_linear_field_coefficients(
    const expr_t *field,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    expr_t **x_coefficient_out,
    expr_t **y_coefficient_out)
{
    expr_t *without_x = NULL;
    expr_t *constant = NULL;
    bool valid = de_linear_decompose(
            field, x, x_coefficient_out, &without_x) &&
        de_linear_decompose(
            without_x, y, y_coefficient_out, &constant) &&
        de_pde_is_characteristic_constant(
            *x_coefficient_out, x, y, dependent) &&
        de_pde_is_characteristic_constant(
            *y_coefficient_out, x, y, dependent) &&
        expr_is_exact_zero(constant);
...
static expr_t *de_pde_trace_zero_linear_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    ...
    const expr_t *y_field)
{
    ...
    cx_squared = c && x_squared
        ? expr_mul_simplify_owned(expr_clone(c), x_squared)
        : NULL;
    x_squared = NULL;
    two_a_xy = a && xy
        ? expr_mul_simplify_owned(expr_mul_long(a, 2L), xy)
        : NULL;
    xy = NULL;
    b_y_squared = b && y_squared
        ? expr_mul_simplify_owned(expr_clone(b), y_squared)
        : NULL;
    y_squared = NULL;
    first_difference = cx_squared && two_a_xy
        ? expr_sub_simplify_owned(cx_squared, two_a_xy)
        : NULL;
    cx_squared = NULL;
    two_a_xy = NULL;
    invariant = first_difference && b_y_squared
        ? expr_sub_simplify_owned(first_difference, b_y_squared)
        : NULL;
    ...
    if (invariant &&
        !de_expr_uses(invariant, x) &&
        !de_expr_uses(invariant, y)) {
        expr_free(invariant);
        invariant = NULL;
    }
...
static expr_t *de_pde_separable_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient)
{
    expr_t *x_potential = de_pde_unit_characteristic_potential(
        x,
        y,
        x,
        x_coefficient);
    expr_t *y_potential = de_pde_unit_characteristic_potential(
        x,
        y,
        y,
        y_coefficient);
    expr_t *invariant = x_potential && y_potential
        ? expr_sub_simplify_owned(y_potential, x_potential)
        : NULL;
...
static expr_t *de_pde_cross_coordinate_invariant_candidate(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
    ...
    const expr_t *denominator)
{
    expr_t *rate = expr_div_simplify_owned(
        expr_clone(numerator), expr_clone(denominator));
    expr_t *primitive = rate &&
            de_expr_uses(rate, integration_variable) &&
            !de_expr_uses(rate, leading_coordinate)
        ? expr_integrate(rate, integration_variable)
        : NULL;
    expr_t *candidate = primitive
        ? expr_sub_simplify_owned(
              expr_clone(leading_coordinate), primitive)
        : NULL;
    expr_t *candidate_x = candidate
        ? expr_create_deriv(candidate, x)
        : NULL;
...
static expr_t *de_pde_cross_coordinate_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient)
{
    expr_t *invariant =
        de_pde_cross_coordinate_invariant_candidate(
            x,
            y,
            x_coefficient,
            y_coefficient,
    ...
    if (!invariant)
        invariant = de_pde_cross_coordinate_invariant_candidate(
            x,
            y,
            x_coefficient,
            y_coefficient,
...
static expr_t *de_pde_linear_characteristic_invariant(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient)
{
    expr_t *invariant = de_pde_monomial_invariant(
        x,
        y,
        dependent,
        x_coefficient,
    ...
    if (!invariant)
        invariant = de_pde_trace_zero_linear_invariant(
            ...
            y_coefficient);
    if (!invariant)
        invariant = de_pde_radial_invariant(
            x, y, x_coefficient, y_coefficient);
    if (!invariant)
        invariant = de_pde_scaled_coordinate_invariant(
            x, y, x_coefficient, y_coefficient);
    if (!invariant)
        invariant = de_pde_cross_coordinate_invariant(
            x, y, x_coefficient, y_coefficient);
    if (!invariant)
        invariant = de_pde_separable_invariant(
            x, y, x_coefficient, y_coefficient);
    return invariant;
}

static expr_t *de_pde_characteristic_exponent(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *reaction_coefficient)
{
    ...
    expr_t *target_x =
        target ? expr_create_deriv(target, x) : NULL;
    expr_t *target_y =
        target ? expr_create_deriv(target, y) : NULL;
    expr_t *target_along_x = target_x
        ? expr_mul(x_coefficient, target_x)
        : NULL;
    expr_t *target_along_y = target_y
        ? expr_mul(y_coefficient, target_y)
        : NULL;
    expr_t *target_directional_raw =
        target_along_x && target_along_y
            ? expr_add(target_along_x, target_along_y)
            : NULL;
    expr_t *target_directional = target_directional_raw
        ? expr_simplify(target_directional_raw)
        : NULL;
    expr_t *target_square = target && target_directional &&
            de_pde_is_characteristic_constant(
                target_directional, x, y, dependent) &&
            !expr_is_exact_zero(target_directional)
        ? expr_pow_long(target, 2L)
        : NULL;
    expr_t *twice_target_directional = target_directional
        ? expr_mul_long(target_directional, 2L)
        : NULL;
    expr_t *phase_exponent = NULL;

    if (target_square && twice_target_directional) {
        phase_exponent = expr_div_simplify_owned(
            target_square, twice_target_directional);
        target_square = NULL;
        twice_target_directional = NULL;
    }
    expr_t *rate_raw =
        target ? expr_div(target, x_coefficient) : NULL;
    expr_t *rate = rate_raw ? expr_simplify(rate_raw) : NULL;
    expr_t *primitive = rate ? expr_integrate(rate, x) : NULL;
    expr_t *primitive_x =
        primitive ? expr_create_deriv(primitive, x) : NULL;
    expr_t *primitive_y =
        primitive ? expr_create_deriv(primitive, y) : NULL;
    expr_t *along_x = primitive_x
        ? expr_mul(x_coefficient, primitive_x)
        : NULL;
    expr_t *along_y = primitive_y
        ? expr_mul(y_coefficient, primitive_y)
        : NULL;
    expr_t *directional_raw = along_x && along_y
        ? expr_add(along_x, along_y)
        : NULL;
    expr_t *directional = directional_raw
        ? expr_display_expanded(directional_raw)
        : NULL;
    expr_t *scale_raw = target && directional
        ? expr_div(target, directional)
        : NULL;
    expr_t *scale = scale_raw ? expr_simplify(scale_raw) : NULL;
    expr_t *exponent_raw = scale &&
            de_pde_is_characteristic_constant(
                scale, x, y, dependent)
        ? expr_mul_simplify_owned(
              expr_clone(scale), expr_clone(primitive))
        : NULL;
    expr_t *exponent = phase_exponent
        ? expr_simplify(phase_exponent)
        : (exponent_raw ? expr_simplify(exponent_raw) : NULL);
    expr_t *exponent_x =
        exponent ? expr_create_deriv(exponent, x) : NULL;
    expr_t *exponent_y =
        exponent ? expr_create_deriv(exponent, y) : NULL;
    expr_t *check_x = exponent_x
        ? expr_mul(x_coefficient, exponent_x)
        : NULL;
    expr_t *check_y = exponent_y
        ? expr_mul(y_coefficient, exponent_y)
        : NULL;
    expr_t *check_sum = check_x && check_y
        ? expr_add(check_x, check_y)
        : NULL;
    expr_t *check_raw = check_sum
        ? expr_add(check_sum, reaction_coefficient)
        : NULL;
    expr_t *check = check_raw ? expr_simplify(check_raw) : NULL;
...
static expr_t *de_pde_characteristic_factor(const expr_t *exponent)
{
    ...
    } else if (expr_match_scaled_expr(
                   exponent, &scale, &scaled_base) &&
               expr_match_log_expr(scaled_base, &log_argument) &&
               num_is_real(scale) &&
               num_is_integer(scale)) {
...
static expr_t *de_pde_apply_characteristic_factor(
    const expr_t *factor,
    const expr_t *arbitrary)
{
    ...
    if (!reciprocal &&
        ...
        num_eq(exponent, NUM_NEG_ONE)) {
        reciprocal = true;
        denominator = power_base;
    }
    num_destroy(&exponent);
    raw = reciprocal
        ? expr_div(arbitrary, denominator)
        : expr_mul(factor, arbitrary);
    result = raw ? expr_simplify(raw) : NULL;
    expr_free(raw);
    return result;
}

static expr_t *de_pde_apply_linear_characteristic_operator(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
    ...
    const expr_t *candidate)
{
    ...
    expr_t *along_y = candidate_y
        ? expr_mul(y_coefficient, candidate_y)
        : NULL;
    expr_t *transport = along_x && along_y
        ? expr_add(along_x, along_y)
        : NULL;
    expr_t *reaction = candidate
        ? expr_mul(reaction_coefficient, candidate)
        : NULL;
    expr_t *applied_raw = transport && reaction
        ? expr_add(transport, reaction)
        : NULL;
    expr_t *applied = applied_raw
        ? expr_simplify(applied_raw)
        : NULL;
...
static bool de_pde_matches_expanded_cotangent(
    const expr_t *candidate,
    const expr_t *cotangent)
{
    ...
    quotient_raw = cos_argument && sin_argument
        ? expr_div(cos_argument, sin_argument)
        : NULL;
...
static bool de_pde_same_transport_value(
    const expr_t *left,
    const expr_t *right)
{
    return de_pde_same_symbolic_form(left, right) ||
        de_pde_matches_expanded_cotangent(left, right) ||
...
static expr_t *de_pde_unit_characteristic_potential(
    ...
    const expr_t *integration_coefficient)
{
    const expr_t *other_variable = expr_struct_eq(integration_variable, x)
        ? y
        : x;
    expr_t *reciprocal = expr_div_simplify_owned(
        expr_const_one(), expr_clone(integration_coefficient));
    expr_t *potential = reciprocal &&
            !de_expr_uses(reciprocal, x) &&
            !de_expr_uses(reciprocal, y)
        ? expr_mul_simplify_owned(
              expr_clone(reciprocal), expr_clone(integration_variable))
        : (reciprocal
              ? expr_integrate(reciprocal, integration_variable)
              : NULL);
    expr_t *derivative = potential
        ? expr_create_deriv(potential, integration_variable)
        : NULL;
    expr_t *ratio_raw = derivative && reciprocal
        ? expr_div(derivative, reciprocal)
        : NULL;
    expr_t *ratio = ratio_raw ? expr_simplify(ratio_raw) : NULL;
    expr_t *check = derivative && reciprocal
        ? expr_sub_simplify_owned(
              derivative, expr_clone(reciprocal))
        : NULL;
    long ratio_value = 0L;
    bool derivative_matches =
        (check && expr_is_exact_zero(check)) ||
        (ratio && de_pde_exact_long(ratio, &ratio_value) &&
         ratio_value == 1L);

    derivative = NULL;
    if (!derivative_matches ||
        de_expr_uses(potential, other_variable)) {
        expr_free(potential);
        potential = NULL;
    }
    expr_free(check);
...
static bool de_pde_quadratic_characteristic_solutions(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    ...
    size_t *solution_count_out)
{
    ...
    *solution_count_out = 0u;
    ...
    if (!dependent_squared ||
        !de_linear_decompose(
            remainder,
            dependent_squared,
            &quadratic_coefficient,
            &forcing) ||
        !expr_is_exact_zero(forcing) ||
        !de_pde_is_characteristic_constant(
            quadratic_coefficient, x, y, dependent) ||
        expr_is_exact_zero(quadratic_coefficient))
        goto cleanup;

    x_potential = de_pde_unit_characteristic_potential(
        x,
        y,
        x,
        x_coefficient);
    y_potential = de_pde_unit_characteristic_potential(
        x,
        y,
        y,
        y_coefficient);
    invariant = x_potential && y_potential
        ? expr_sub_simplify_owned(
              expr_clone(y_potential), expr_clone(x_potential))
        : NULL;
    arbitrary = invariant
        ? expr_new_arbitrary_function("F", invariant)
        : NULL;
    scaled_potential = x_potential
        ? expr_mul_simplify_owned(
              expr_clone(quadratic_coefficient),
              expr_clone(x_potential))
        : NULL;
    if (de && de->condition_count > 0u) {
        solutions_out[0] = de_pde_quadratic_boundary_solution(
            de,
            x,
            y,
            dependent,
    ...
    denominator = scaled_potential && arbitrary
        ? expr_add_simplify_owned(scaled_potential, arbitrary)
        : NULL;
    scaled_potential = NULL;
    arbitrary = NULL;
    right = denominator
        ? expr_div_simplify_owned(expr_const_one(), denominator)
        : NULL;
    denominator = NULL;
    if (!right)
        goto cleanup;
...
static expr_t *de_pde_characteristic_particular_along(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *integration_coefficient)
{
    ...
    expr_t *rate_raw = target
        ? expr_div(target, integration_coefficient)
        : NULL;
    expr_t *rate = rate_raw ? expr_simplify(rate_raw) : NULL;
    expr_t *primitive = rate ? expr_integrate(rate, integration_variable) : NULL;
    expr_t *applied = primitive
        ? de_pde_apply_linear_characteristic_operator(
              x,
              y,
              x_coefficient,
              y_coefficient,
              ...
              primitive)
        : NULL;
    bool primitive_matches = target && applied &&
        de_pde_same_transport_value(applied, target);
    expr_t *scale_raw = primitive_matches
        ? expr_const_one()
        : (target && applied && !expr_is_exact_zero(applied)
              ? expr_div(target, applied)
              : NULL);
    expr_t *scale = scale_raw ? expr_simplify(scale_raw) : NULL;
    expr_t *candidate_raw = scale &&
            de_pde_is_characteristic_constant(
                scale, x, y, dependent)
        ? expr_mul(scale, primitive)
        : NULL;
    expr_t *candidate = candidate_raw
        ? expr_simplify(candidate_raw)
        : NULL;
    expr_t *candidate_applied = candidate
        ? de_pde_apply_linear_characteristic_operator(
              x,
              y,
              x_coefficient,
              ...
              candidate)
        : NULL;
    bool verified = candidate_applied && target &&
        de_pde_same_transport_value(candidate_applied, target);
...
static expr_t *de_pde_characteristic_eigen_particular(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
...
{
    expr_t *target = expr_negate_owned(expr_clone(forcing));
    expr_t *applied = de_pde_apply_linear_characteristic_operator(
        x,
        y,
        x_coefficient,
        y_coefficient,
        reaction_coefficient,
        forcing);
    expr_t *eigenvalue_raw = applied && forcing
        ? expr_div(applied, forcing)
        : NULL;
    expr_t *eigenvalue = eigenvalue_raw
        ? expr_simplify(eigenvalue_raw)
        : NULL;
    expr_t *candidate_raw = target &&
            eigenvalue &&
            !expr_is_exact_zero(eigenvalue) &&
            de_pde_is_characteristic_constant(
                eigenvalue, x, y, dependent)
...
{
    static const unsigned char x_power[DE_PDE_QUAD_COUNT] = {
        0u, 1u, 0u, 2u, 1u, 0u
    };
    static const unsigned char y_power[DE_PDE_QUAD_COUNT] = {
        0u, 0u, 1u, 0u, 1u, 2u
    };
...
static bool de_pde_quadratic_multiply(
    const number_t *left,
...
static bool de_pde_collect_quadratic(
    const expr_t *expr,
    const expr_t *x,
    const expr_t *y,
    number_t *out)
{
    ...
    if (expr_struct_eq(expr, x) || expr_struct_eq(expr, y)) {
        size_t index = expr_struct_eq(expr, x)
            ? DE_PDE_QUAD_X
            : DE_PDE_QUAD_Y;
    ...
    if (expr_match_add_sub_expr(expr, &left, &right, &is_sub)) {
        ...
        ok = de_pde_collect_quadratic(left, x, y, left_poly) &&
             de_pde_collect_quadratic(right, x, y, right_poly);
        if (ok) {
            for (size_t i = 0u; i < DE_PDE_QUAD_COUNT; ++i) {
                number_t sum = is_sub
                    ? num_sub(left_poly[i], right_poly[i])
...
static bool de_pde_quadratic_scale(
    const expr_t *target,
    const expr_t *applied,
    const expr_t *x,
    const expr_t *y,
    number_t *scale_out)
{
    ...
    matches = de_pde_collect_quadratic(target, x, y, target_poly) &&
              de_pde_collect_quadratic(applied, x, y, applied_poly);
    for (size_t i = 0u;
         matches && i < DE_PDE_QUAD_COUNT;
         ++i) {
...
static expr_t *de_pde_characteristic_scaled_trial(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *trial)
{
    expr_t *applied = de_pde_apply_linear_characteristic_operator(
        ...
        reaction_coefficient,
        trial);
    expr_t *scale_raw = applied && !expr_is_exact_zero(applied)
        ? expr_div(target, applied)
        : NULL;
    expr_t *scale = scale_raw ? expr_simplify(scale_raw) : NULL;
    ...
    if ((!scale ||
         !de_pde_is_characteristic_constant(
             scale, x, y, dependent)) &&
        applied &&
        de_pde_quadratic_scale(
            target, applied, x, y, &polynomial_scale)) {
        polynomial_scale_expr = expr_new_const(polynomial_scale);
    }
    candidate_raw =
        (polynomial_scale_expr ||
         (scale &&
          de_pde_is_characteristic_constant(
              scale, x, y, dependent)))
...
static expr_t *de_pde_characteristic_basis_particular(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *forcing)
{
    ...
    expr_t *trials[5] = {
        ...
        expr_clone(y),
        expr_pow_long(x, 2L),
        expr_mul_simplify_owned(expr_clone(x), expr_clone(y)),
        expr_pow_long(y, 2L)
    };
    ...
    for (size_t i = 0u; target && i < 5u && !particular; ++i) {
        if (!trials[i])
            continue;
        particular = de_pde_characteristic_scaled_trial(
            x,
            y,
            dependent,
            x_coefficient,
...
static expr_t *de_pde_characteristic_invariant_particular_with_potential(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
    ...
    const expr_t *potential)
{
    ...
    expr_t *target_directional = zero && target
        ? de_pde_apply_linear_characteristic_operator(
              x,
              y,
              x_coefficient,
              ...
              target)
        : NULL;
    expr_t *candidate_raw = potential && target
        ? expr_mul(potential, target)
        : NULL;
    expr_t *candidate = candidate_raw
        ? expr_simplify(candidate_raw)
        : NULL;
    bool verified = expr_is_exact_zero(reaction_coefficient) &&
        target_directional &&
        expr_is_exact_zero(target_directional);
...
static expr_t *de_pde_characteristic_invariant_particular_along(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
    ...
    const expr_t *integration_coefficient)
{
    expr_t *potential = de_pde_unit_characteristic_potential(
        x,
        y,
        integration_variable,
        integration_coefficient);
    expr_t *candidate = potential
...
static expr_t *de_pde_characteristic_invariant_particular(
    const expr_t *x,
    const expr_t *y,
    const expr_t *x_coefficient,
    const expr_t *y_coefficient,
    ...
    const expr_t *forcing)
{
    ...
    expr_t *y_potential = de_pde_unit_characteristic_potential(
        x, y, y, y_coefficient);
    expr_t *potential_sum = x_potential && y_potential
        ? expr_add_simplify_owned(
              expr_clone(x_potential), expr_clone(y_potential))
        : NULL;
    expr_t *average = potential_sum
        ? expr_div_long(potential_sum, 2L)
        : NULL;
    ...
    if (!particular && target && x_potential)
        particular = de_pde_characteristic_invariant_particular_along(
            x,
            y,
            x_coefficient,
            y_coefficient,
...
static expr_t *de_pde_characteristic_particular(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *forcing)
{
    expr_t *particular = de_pde_characteristic_eigen_particular(
        x,
        y,
        dependent,
        x_coefficient,
    ...
    if (!particular)
        particular = de_pde_characteristic_basis_particular(
            x,
            y,
            dependent,
            x_coefficient,
            ...
            forcing);
    if (!particular)
        particular = de_pde_characteristic_invariant_particular(
            x,
            y,
            x_coefficient,
            y_coefficient,
            ...
            forcing);
    if (!particular)
        particular = de_pde_characteristic_particular_along(
            x,
            y,
            dependent,
            x_coefficient,
...
static expr_t *de_pde_remove_dependent_factor(
    ...
    const expr_t *dependent)
{
    ...
    if (left_uses == right_uses)
        return NULL;
    reduced = de_pde_remove_dependent_factor(
        left_uses ? left : right, dependent);
    return reduced
        ? expr_mul_simplify_owned(
              reduced, expr_clone(left_uses ? right : left))
        : NULL;
}

static bool de_pde_dependent_square_solutions(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    ...
    size_t *solution_count_out)
{
    ...
    if (!de_expr_uses(remainder, dependent)) {
        base_x = de_pde_remove_dependent_factor(
            x_coefficient, dependent);
        base_y = de_pde_remove_dependent_factor(
            y_coefficient, dependent);
        transformed_forcing = base_x && base_y
            ? expr_mul_long(remainder, 2L)
            : NULL;
    } else if (!de_expr_uses(x_coefficient, dependent) &&
               !de_expr_uses(y_coefficient, dependent)) {
        scaled_remainder_raw = expr_mul(remainder, dependent);
        scaled_remainder = scaled_remainder_raw
            ? expr_simplify(scaled_remainder_raw)
            : NULL;
        if (scaled_remainder &&
            !de_expr_uses(scaled_remainder, dependent)) {
            base_x = expr_clone(x_coefficient);
            base_y = expr_clone(y_coefficient);
            transformed_forcing = expr_mul_long(
                scaled_remainder, 2L);
        }
    }
    invariant = base_x && base_y && transformed_forcing
        ? de_pde_linear_characteristic_invariant(
              x, y, dependent, base_x, base_y)
        : NULL;
    arbitrary = invariant
        ? expr_new_arbitrary_function("F", invariant)
        : NULL;
    particular = arbitrary && transformed_forcing && zero &&
            !expr_is_exact_zero(transformed_forcing)
        ? de_pde_characteristic_particular(
              ...
              transformed_forcing)
        : NULL;
    squared_right = arbitrary && expr_is_exact_zero(transformed_forcing)
        ? expr_clone(arbitrary)
        : (arbitrary && particular
              ? expr_add_simplify_owned(
                    expr_clone(arbitrary), expr_clone(particular))
              : NULL);
    if (de && de->condition_count > 0u) {
        expr_t *zero_particular = particular
            ? NULL
            : expr_const_zero();

        solutions_out[0] = de_pde_dependent_square_boundary_solution(
            de,
            x,
            y,
            dependent,
    ...
    root_raw = squared_right
        ? expr_sqrt(squared_right)
        : NULL;
    root = root_raw ? expr_simplify(root_raw) : NULL;
    negative_root = root
        ? expr_negate_owned(expr_clone(root))
        : NULL;

    if (root && negative_root) {
        solutions_out[0] = de_pde_solution_equation(dependent, root);
        solutions_out[1] =
            de_pde_solution_equation(dependent, negative_root);
        solved = solutions_out[0] && solutions_out[1];
        if (solved)
            *solution_count_out = 2u;
    }
...
static expr_t *de_pde_substitute_characteristic_curve(
    const expr_t *expr,
    const expr_t *x,
    const expr_t *y,
    const expr_t *curve_x,
    const expr_t *curve_y)
{
    expr_t *after_x = expr_substitute(expr, x, curve_x);
    expr_t *after_y = after_x
        ? expr_substitute(after_x, y, curve_y)
        : NULL;
...
static bool de_pde_affine_parameter_components(
    const expr_t *expr,
    ...
    expr_t **constant_out)
{
    ...
    *constant_out = NULL;
    ...
    if (de_linear_decompose(
            expr, parameter, &coefficient, &constant) &&
        !de_expr_uses(coefficient, parameter) &&
...
static expr_t *de_pde_boundary_parameter_value(
    const expr_t *boundary_invariant,
    const expr_t *parameter,
    const expr_t *current_invariant)
{
    ...
    if (!boundary_invariant || !parameter || !current_invariant)
        return NULL;
    if (de_pde_affine_parameter_components(
            boundary_invariant,
            parameter,
            &coefficient,
            &constant) &&
        ...
        !de_expr_uses(coefficient, parameter)) {
        if (de_pde_exact_long(
                coefficient, &exponent_coefficient) &&
            exponent_coefficient == -1L) {
            value = expr_sub_simplify_owned(
                constant, expr_clone(current_invariant));
            constant = NULL;
            goto cleanup;
        }
        numerator = expr_sub_simplify_owned(
            expr_clone(current_invariant), constant);
        constant = NULL;
        if (numerator &&
            de_pde_exact_long(
                coefficient, &exponent_coefficient) &&
            ...
            value = numerator
                ? expr_div_simplify_owned(numerator, coefficient)
                : NULL;
            numerator = NULL;
            coefficient = NULL;
        }
    ...
    if (expr_match_exp_expr(boundary_invariant, &exponent) &&
        de_pde_affine_parameter_components(
            exponent,
            parameter,
            &coefficient,
            &constant) &&
        ...
        numerator = logarithm
            ? expr_sub_simplify_owned(logarithm, constant)
            : NULL;
        logarithm = NULL;
        constant = NULL;
        if (numerator &&
    ...
    parameter_from_reciprocal = reciprocal_coordinate
        ? expr_div_simplify_owned(
              expr_const_one(), expr_clone(reciprocal_coordinate))
        : NULL;
    transformed_raw = parameter_from_reciprocal
        ? expr_substitute(
              boundary_invariant,
              parameter,
    ...
    transformed = transformed_raw
        ? expr_display_expanded(transformed_raw)
        : NULL;
    if (reciprocal_coordinate && transformed &&
        ...
        constant = NULL;
        value = numerator
            ? expr_div_simplify_owned(coefficient, numerator)
            : NULL;
        coefficient = NULL;
        numerator = NULL;
        goto cleanup;
...
static expr_t *de_pde_boundary_parameter_squared_value(
    const expr_t *boundary_invariant,
    const expr_t *parameter,
    const expr_t *current_invariant)
{
    ...
    if (expr_match_div_expr(
        ...
        expr_match_div_expr(
            ...
            &inner_numerator,
            &inner_denominator) &&
        expr_struct_eq(inner_denominator, parameter) &&
        !de_expr_uses(inner_numerator, parameter)) {
        value = expr_div_simplify_owned(
            expr_clone(inner_numerator),
            expr_clone(current_invariant));
        goto cleanup;
    }
    ...
    if (parameter_squared &&
        ...
        de_pde_affine_parameter_components(
            ...
            &constant) &&
        !expr_is_exact_zero(coefficient)) {
        current_reciprocal = expr_div_simplify_owned(
            expr_clone(outer_numerator),
            expr_clone(current_invariant));
        numerator = current_reciprocal
            ? expr_sub_simplify_owned(current_reciprocal, constant)
            : NULL;
    ...
    constant = NULL;
    coefficient = NULL;
    scaled_invariant_raw = parameter_squared
        ? expr_mul(boundary_invariant, parameter_squared)
        : NULL;
    scaled_invariant = scaled_invariant_raw
        ? expr_simplify(scaled_invariant_raw)
        : NULL;
    if (scaled_invariant &&
        !de_expr_uses(scaled_invariant, parameter) &&
        !expr_is_exact_zero(scaled_invariant)) {
        value = expr_div_simplify_owned(
            expr_clone(scaled_invariant),
            expr_clone(current_invariant));
        goto cleanup;
    }
    if (parameter_squared &&
    ...
    reciprocal_invariant = expr_div_simplify_owned(
        expr_const_one(), expr_clone(boundary_invariant));
    if (reciprocal_invariant && parameter_squared &&
        de_pde_affine_parameter_components(
            reciprocal_invariant,
            parameter_squared,
            &coefficient,
            &constant) &&
        !expr_is_exact_zero(coefficient)) {
        current_reciprocal = expr_div_simplify_owned(
            expr_const_one(), expr_clone(current_invariant));
        numerator = current_reciprocal
            ? expr_sub_simplify_owned(current_reciprocal, constant)
            : NULL;
        current_reciprocal = NULL;
    ...
    reciprocal_square = parameter_squared
        ? expr_div_simplify_owned(
              expr_const_one(), expr_clone(parameter_squared))
        : NULL;
    ...
    const expr_t *particular)
{
    ...
    for (size_t condition_index = 0u;
        ...
        if (!left || !value ||
            ...
            de->condition_point_counts[condition_index] != 2u)
            continue;
        ...
        for (size_t parameter_index = 0u;
            ...
            expr_t *curve_x = dummy
                ? expr_substitute(point_x, parameter, dummy)
                : NULL;
            expr_t *curve_y = dummy
                ? expr_substitute(point_y, parameter, dummy)
                : NULL;
            expr_t *curve_value = dummy
                ? expr_substitute(value, parameter, dummy)
                : NULL;
            ...
            if (!curve_x || !curve_y || !curve_value ||
                de_expr_uses(curve_x, other) ||
                de_expr_uses(curve_y, other) ||
                de_expr_uses(curve_value, other) ||
                (!de_expr_uses(curve_x, dummy) &&
                 !de_expr_uses(curve_y, dummy)))
                goto candidate_cleanup;
            boundary_invariant =
                de_pde_substitute_characteristic_curve(
                    invariant, x, y, curve_x, curve_y);
            boundary_particular_raw =
                de_pde_substitute_characteristic_curve(
                    particular, x, y, curve_x, curve_y);
            boundary_particular_curve = boundary_particular_raw
                ? expr_simplify(boundary_particular_raw)
                : NULL;
            parameter_squared = expr_pow_long(dummy, 2L);
            parameter_squared_value =
                de_pde_boundary_parameter_squared_value(
                    boundary_invariant, dummy, invariant);
            boundary_value_squared_curve = expr_pow_long(
                curve_value, 2L);
            if (parameter_squared_value && parameter_squared &&
                boundary_value_squared_curve) {
                boundary_value_squared = expr_struct_eq(
                        boundary_value_squared_curve,
                        parameter_squared)
                ...
                boundary_particular = boundary_particular_curve
                    ? (!de_expr_uses(boundary_particular_curve, dummy)
                          ...
                          : expr_substitute(
                                ...
                                parameter_squared_value))
                    : NULL;
                if ((boundary_value_squared &&
                     de_expr_uses(boundary_value_squared, dummy)) ||
                    (boundary_particular &&
                     de_expr_uses(boundary_particular, dummy))) {
            ...
            if (!boundary_value_squared || !boundary_particular) {
                parameter_value = de_pde_boundary_parameter_value(
                    boundary_invariant, dummy, invariant);
                if (!parameter_value)
                    goto candidate_cleanup;
                boundary_particular = boundary_particular_curve
                    ? expr_substitute(
                          ...
                          parameter_value)
                    : NULL;
                boundary_value = expr_substitute(
                    curve_value, dummy, parameter_value);
                boundary_value_squared = boundary_value
                    ? expr_pow_long(boundary_value, 2L)
                    : NULL;
            }
            arbitrary_value = boundary_value_squared &&
                    boundary_particular
                ? expr_sub_simplify_owned(
                      boundary_value_squared, boundary_particular)
                : NULL;
            boundary_value_squared = NULL;
            boundary_particular = NULL;
            squared_right = arbitrary_value
                ? expr_add_simplify_owned(
                      expr_clone(particular), arbitrary_value)
                : NULL;
            ...
            right = root_raw ? expr_simplify(root_raw) : NULL;
            {
                ...
                if (right && expr_match_neg_expr(
                        curve_value, &positive_boundary_value))
    ...
    const expr_t *scaled_potential)
{
    ...
    for (size_t condition_index = 0u;
        ...
        for (size_t parameter_index = 0u;
            ...
            if (!parameter_value)
                goto candidate_cleanup;
            boundary_potential_curve =
                de_pde_substitute_characteristic_curve(
                    scaled_potential, x, y, curve_x, curve_y);
            boundary_potential = boundary_potential_curve
                ? expr_substitute(
                      boundary_potential_curve, dummy, parameter_value)
                : NULL;
            boundary_value = expr_substitute(
                curve_value, dummy, parameter_value);
            reciprocal_value = boundary_value
                ? expr_div_simplify_owned(
                      expr_const_one(), boundary_value)
                : NULL;
            boundary_value = NULL;
            arbitrary_value = reciprocal_value && boundary_potential
                ? expr_sub_simplify_owned(
                      reciprocal_value, boundary_potential)
                : NULL;
            reciprocal_value = NULL;
            boundary_potential = NULL;
            denominator = arbitrary_value
                ? expr_add_simplify_owned(
                      expr_clone(scaled_potential), arbitrary_value)
                : NULL;
            arbitrary_value = NULL;
            right = denominator
                ? expr_div_simplify_owned(
                      expr_const_one(), denominator)
                : NULL;
            denominator = NULL;
            solution = right
                ? de_pde_solution_equation(dependent, right)
...
static expr_t *de_pde_boundary_curve_value(
    const expr_t *curve_expression,
    const expr_t *parameter,
    const expr_t *parameter_value,
    const expr_t *parameter_squared,
    const expr_t *parameter_squared_value)
{
    ...
    if (parameter_value)
        return expr_substitute(
            curve_expression, parameter, parameter_value);
...
static equation_t *de_pde_parametric_boundary_solution(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
        ...
        for (size_t parameter_index = 0u;
            ...
            if (!curve_x || !curve_y || !curve_value ||
                ...
                (!de_expr_uses(curve_x, dummy) &&
                 !de_expr_uses(curve_y, dummy)))
                goto candidate_cleanup;
            boundary_invariant_raw =
                de_pde_substitute_characteristic_curve(
                    invariant, x, y, curve_x, curve_y);
            boundary_invariant = boundary_invariant_raw
                ? expr_simplify(boundary_invariant_raw)
                : NULL;
            parameter_value = de_pde_boundary_parameter_value(
                boundary_invariant, dummy, invariant);
            if (!parameter_value) {
                ...
                parameter_squared_value =
                    de_pde_boundary_parameter_squared_value(
                        boundary_invariant, dummy, invariant);
                parameter_root_raw = parameter_squared_value
                    ? expr_sqrt(parameter_squared_value)
                    : NULL;
                parameter_value = parameter_root_raw
                    ? expr_simplify(parameter_root_raw)
                    : NULL;
            }
            if (!parameter_value && !parameter_squared_value)
                goto candidate_cleanup;
            boundary_factor_curve =
                de_pde_substitute_characteristic_curve(
                    factor, x, y, curve_x, curve_y);
            boundary_factor = de_pde_boundary_curve_value(
                boundary_factor_curve,
                dummy,
                parameter_value,
                parameter_squared,
            ...
            boundary_particular_curve = particular
                ? de_pde_substitute_characteristic_curve(
                      particular, x, y, curve_x, curve_y)
                : expr_const_zero();
            boundary_particular = de_pde_boundary_curve_value(
                boundary_particular_curve,
                dummy,
                parameter_value,
                parameter_squared,
                parameter_squared_value);
            boundary_value = de_pde_boundary_curve_value(
                curve_value,
                dummy,
                parameter_value,
                parameter_squared,
                parameter_squared_value);
            if (!particular) {
                factor_ratio = factor && boundary_factor
                    ? expr_div_simplify_owned(
                          expr_clone(factor), boundary_factor)
                    : NULL;
                boundary_factor = NULL;
                right = factor_ratio && boundary_value
                    ? expr_mul_simplify_owned(
                          factor_ratio, boundary_value)
                    : NULL;
                factor_ratio = NULL;
                boundary_value = NULL;
                solution = right
            ...
            boundary_delta = boundary_value && boundary_particular
                ? expr_sub_simplify_owned(
                      boundary_value, boundary_particular)
                : NULL;
            boundary_value = NULL;
            boundary_particular = NULL;
            arbitrary_value = boundary_delta && boundary_factor
                ? expr_div_simplify_owned(
                      boundary_delta, boundary_factor)
                : NULL;
            boundary_delta = NULL;
            boundary_factor = NULL;
            homogeneous = factor && arbitrary_value
                ? expr_mul_simplify_owned(
                      expr_clone(factor), arbitrary_value)
                : NULL;
            arbitrary_value = NULL;
            right = homogeneous && particular
                ? expr_add_simplify_owned(
                      homogeneous, expr_clone(particular))
                : homogeneous;
            homogeneous = NULL;
            solution = right
                ? de_pde_solution_equation(dependent, right)
...
static equation_t *de_pde_variable_linear_solution(
    const diffequ_t *de,
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    ...
    const expr_t *forcing)
{
    expr_t *invariant = de_pde_linear_characteristic_invariant(
        x,
        y,
        dependent,
        x_coefficient,
    ...
    if (expr_is_exact_zero(reaction_coefficient)) {
        ...
        exponent = de_pde_characteristic_exponent(
            x,
            y,
            dependent,
            x_coefficient,
        ...
        homogeneous = factor
            ? de_pde_apply_characteristic_factor(factor, arbitrary)
            : NULL;
    }
    if (!homogeneous)
        goto cleanup;
    if (!expr_is_exact_zero(forcing)) {
        particular = de_pde_characteristic_particular(
            x,
            y,
            dependent,
            x_coefficient,
    ...
    if (de && de->condition_count > 0u) {
        solution = de_pde_parametric_boundary_solution(
            de,
            x,
            y,
            dependent,
    ...
    if (expr_is_exact_zero(forcing)) {
        ...
        right = particular
            ? expr_add_simplify_owned(
                  expr_clone(homogeneous), expr_clone(particular))
            : NULL;
    }
    solution = right
        ? de_pde_solution_equation(dependent, right)
...
static bool de_pde_is_first_integral(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *candidate)
{
    expr_t *candidate_x = expr_create_deriv(candidate, x);
    expr_t *candidate_y = expr_create_deriv(candidate, y);
    expr_t *candidate_dependent =
        expr_create_deriv(candidate, dependent);
    expr_t *along_x = candidate_x
        ? expr_mul(x_coefficient, candidate_x)
        : NULL;
    expr_t *along_y = candidate_y
        ? expr_mul(y_coefficient, candidate_y)
        : NULL;
    expr_t *along_dependent = candidate_dependent
        ? expr_mul(dependent_coefficient, candidate_dependent)
        : NULL;
    expr_t *xy_sum = along_x && along_y
        ? expr_add(along_x, along_y)
        : NULL;
    expr_t *directional_raw = xy_sum && along_dependent
        ? expr_add(xy_sum, along_dependent)
        : NULL;
    expr_t *directional = directional_raw
        ? expr_simplify(directional_raw)
        : NULL;
    bool is_first_integral =
        directional && expr_is_exact_zero(directional);
...
static bool de_pde_is_scaled_cyclic_field(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    long power)
{
    ...
    expr_t *y_minus_dependent = y_power && dependent_power
        ? expr_sub_simplify_owned(
              expr_clone(y_power), expr_clone(dependent_power))
        : NULL;
    expr_t *dependent_minus_x = dependent_power && x_power
        ? expr_sub_simplify_owned(
              expr_clone(dependent_power), expr_clone(x_power))
        : NULL;
    expr_t *x_minus_y = x_power && y_power
        ? expr_sub_simplify_owned(
              expr_clone(x_power), expr_clone(y_power))
        : NULL;
    expr_t *base_x = y_minus_dependent
        ? expr_mul_simplify_owned(expr_clone(x), y_minus_dependent)
        : NULL;
    expr_t *base_y = dependent_minus_x
        ? expr_mul_simplify_owned(expr_clone(y), dependent_minus_x)
        : NULL;
    expr_t *base_dependent = x_minus_y
        ? expr_mul_simplify_owned(
              expr_clone(dependent), x_minus_y)
        : NULL;
    expr_t *scale_x_raw = base_x
        ? expr_div(x_coefficient, base_x)
        : NULL;
    expr_t *scale_y_raw = base_y
        ? expr_div(y_coefficient, base_y)
        : NULL;
    expr_t *scale_dependent_raw = base_dependent
        ? expr_div(dependent_coefficient, base_dependent)
        : NULL;
    expr_t *scale_x = scale_x_raw
        ? expr_simplify(scale_x_raw)
        : NULL;
    expr_t *scale_y = scale_y_raw
        ? expr_simplify(scale_y_raw)
        : NULL;
    expr_t *scale_dependent = scale_dependent_raw
        ? expr_simplify(scale_dependent_raw)
        : NULL;
    bool matches = scale_x &&
        !expr_is_exact_zero(scale_x) &&
        de_pde_same_symbolic_form(scale_x, scale_y) &&
...
static void de_pde_collect_cyclic_power_candidates(
    const expr_t *expr,
    long *powers,
    size_t *power_count,
    size_t power_capacity)
{
    ...
    if (!expr || !powers || !power_count ||
        *power_count >= power_capacity)
        goto cleanup;
    if (expr_match_pow_const(expr, &base, &exponent)) {
        exponent_expr = expr_new_const(exponent);
        if (exponent_expr &&
            de_pde_exact_long(exponent_expr, &power) &&
            power > 0L) {
    ...
    if (expr_child_exprs(expr, &left, &right)) {
        de_pde_collect_cyclic_power_candidates(
            left, powers, power_count, power_capacity);
        de_pde_collect_cyclic_power_candidates(
            right, powers, power_count, power_capacity);
    }
...
static equation_t *de_pde_cyclic_first_integral_solution(
    const expr_t *x,
    const expr_t *y,
    const expr_t *dependent,
    const expr_t *x_coefficient,
    ...
    const expr_t *remainder)
{
    expr_t *dependent_coefficient =
        expr_negate_owned(expr_clone(remainder));
    expr_t *xy_product =
        expr_mul_simplify_owned(expr_clone(x), expr_clone(y));
    expr_t *product = xy_product
        ? expr_mul_simplify_owned(xy_product, expr_clone(dependent))
        : NULL;
    equation_t *solution = NULL;
    long powers[16] = { 1L };
    ...
    de_pde_collect_cyclic_power_candidates(
        x_coefficient, powers, &power_count, 16u);
    de_pde_collect_cyclic_power_candidates(
        y_coefficient, powers, &power_count, 16u);
    de_pde_collect_cyclic_power_candidates(
        dependent_coefficient, powers, &power_count, 16u);

    for (size_t i = 0u;
         dependent_coefficient && product &&
             i < power_count && !solution;
         ++i) {
        expr_t *x_power = expr_pow_long(x, powers[i]);
        expr_t *y_power = expr_pow_long(y, powers[i]);
        expr_t *dependent_power =
            expr_pow_long(dependent, powers[i]);
        expr_t *xy_sum = x_power && y_power
            ? expr_add_simplify_owned(x_power, y_power)
            : NULL;
        expr_t *sum = xy_sum && dependent_power
            ? expr_add_simplify_owned(xy_sum, dependent_power)
            : NULL;
        expr_t *arguments[2] = { sum, product };
        bool verified = sum &&
            ((de_pde_is_first_integral(
                  x,
                  y,
                  dependent,
                  x_coefficient,
        ...
        if (verified) {
            expr_t *arbitrary_relation =
                expr_new_arbitrary_function_n("F", 2u, arguments);
            expr_t *zero = expr_const_zero();

            solution = arbitrary_relation && zero
                ? equ_new(arbitrary_relation, zero)
                : NULL;
            expr_free(zero);
    ...
    size_t *solution_count_out)
{
    ...
    *solution_count_out = 0u;
    ...
    if (!de_expr_uses(x_coefficient, dependent) &&
        !de_expr_uses(y_coefficient, dependent) &&
        de_linear_decompose(
            remainder,
            dependent,
            ...
            &forcing)) {
        solutions_out[0] =
            de_pde_variable_linear_solution(
                de,
                x,
                y,
                dependent,
    ...
    if (de_pde_quadratic_characteristic_solutions(
            de,
            x,
            y,
            dependent,
    ...
    solutions_out[0] = de_pde_cyclic_first_integral_solution(
        x,
        y,
        dependent,
        x_coefficient,
    ...
    if (de_pde_dependent_square_solutions(
            de,
            x,
            y,
            dependent,
