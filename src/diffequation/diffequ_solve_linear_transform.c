...
static bool de_extract_linear_form(const expr_t *expr,
                                   ...
                                   de_linear_form_t *form)
{
    ...
    valid =
        ...
        !de_expr_uses(form->x_coefficient, independent) &&
        !de_expr_uses(form->x_coefficient, dependent) &&
        !de_expr_uses(form->y_coefficient, independent) &&
        !de_expr_uses(form->y_coefficient, dependent) &&
        (!expr_is_exact_zero(form->x_coefficient) ||
         !expr_is_exact_zero(form->y_coefficient));
    expr_free(constant);
    expr_free(remainder);
    if (!valid)
...
static bool de_linear_forms_contains(const de_linear_forms_t *forms,
                                     const expr_t *expression)
{
    for (size_t i = 0u; i < forms->count; ++i) {
        if (expr_struct_eq(
                forms->forms[i].expression, expression))
            return true;
    }
    return false;
}

static void de_collect_linear_forms(const expr_t *expr,
                                    const expr_t *independent,
                                    const expr_t *dependent,
                                    de_linear_forms_t *forms)
{
    ...
    if (!expr || forms->count >= DE_LINEAR_FORM_LIMIT ||
        expr_is_formal_derivative(expr))
        return;

    if (!de_linear_forms_contains(forms, expr)) {
        de_linear_form_t form = { 0 };

        if (de_extract_linear_form(
                expr, independent, dependent, &form))
            forms->forms[forms->count++] = form;
    }

    if (!expr_child_exprs(expr, &left, &right))
        return;
    de_collect_linear_forms(
        left, independent, dependent, forms);
    de_collect_linear_forms(
        right, independent, dependent, forms);
}

static expr_t *de_linear_determinant(
    ...
    const de_linear_form_t *transformed_independent)
{
    ...
    expr_t *second = expr_mul_simplify_owned(
        expr_clone(transformed_independent->y_coefficient),
        expr_clone(transformed_dependent->x_coefficient));

    return expr_sub_simplify_owned(first, second);
}

static expr_t *de_inverse_x(
    const de_linear_form_t *transformed_dependent,
    const de_linear_form_t *transformed_independent,
    const expr_t *new_independent,
    const expr_t *new_dependent,
    const expr_t *determinant)
{
    ...
    expr_t *y_coefficient = expr_div_simplify_owned(
        de_simplify_unary_owned(
            expr_clone(
                transformed_independent->y_coefficient),
            expr_neg),
        expr_clone(determinant));
    expr_t *x_term = expr_mul_simplify_owned(
        x_coefficient, expr_clone(new_independent));
    expr_t *y_term = expr_mul_simplify_owned(
        y_coefficient, expr_clone(new_dependent));

    return expr_add_simplify_owned(x_term, y_term);
}

static expr_t *de_inverse_y(
    const de_linear_form_t *transformed_dependent,
    const de_linear_form_t *transformed_independent,
    const expr_t *new_independent,
    const expr_t *new_dependent,
    const expr_t *determinant)
{
    expr_t *x_coefficient = expr_div_simplify_owned(
        de_simplify_unary_owned(
            expr_clone(
                transformed_dependent->x_coefficient),
            expr_neg),
        expr_clone(determinant));
    expr_t *y_coefficient = expr_div_simplify_owned(
        expr_clone(transformed_independent->x_coefficient),
...
static expr_t *de_transformed_derivative(
    const expr_t *transformed_rhs,
    const de_linear_form_t *transformed_dependent,
    const de_linear_form_t *transformed_independent)
{
    ...
    if (!expr_match_div_expr(
            transformed_rhs, &rhs_numerator, &rhs_denominator)) {
        rhs_numerator = transformed_rhs;
        rhs_denominator = NULL;
    }

    dependent_base = rhs_denominator
        ? expr_mul_simplify_owned(
              expr_clone(transformed_dependent->x_coefficient),
              expr_clone(rhs_denominator))
        : expr_clone(transformed_dependent->x_coefficient);
    dependent_scaled = expr_mul_simplify_owned(
        expr_clone(transformed_dependent->y_coefficient),
        expr_clone(rhs_numerator));
    numerator = expr_add_simplify_owned(
        dependent_base, dependent_scaled);
    dependent_base = NULL;
    dependent_scaled = NULL;

    independent_base = rhs_denominator
        ? expr_mul_simplify_owned(
              expr_clone(
                  transformed_independent->x_coefficient),
              expr_clone(rhs_denominator))
        : expr_clone(transformed_independent->x_coefficient);
    independent_scaled = expr_mul_simplify_owned(
        expr_clone(transformed_independent->y_coefficient),
        expr_clone(rhs_numerator));
    denominator = expr_add_simplify_owned(
        independent_base, independent_scaled);
...
static expr_t *de_linear_form_at_condition(
    const de_linear_form_t *form,
    const expr_t *point,
    const expr_t *value)
{
    expr_t *x_term = expr_mul_simplify_owned(
        expr_clone(form->x_coefficient), expr_clone(point));
    expr_t *y_term = expr_mul_simplify_owned(
        expr_clone(form->y_coefficient), expr_clone(value));

    return expr_add_simplify_owned(x_term, y_term);
}

static expr_t *de_transformed_constant(
    const diffequ_t *de,
    const expr_t *dependent,
    const de_linear_form_t *transformed_dependent,
    const de_linear_form_t *transformed_independent,
    ...
    const expr_t *independent_integral)
{
    ...
    dependent_value = de_linear_form_at_condition(
        transformed_dependent, point, value);
    independent_value = de_linear_form_at_condition(
        transformed_independent, point, value);
    dependent_at_condition = dependent_value
        ? expr_substitute(
              dependent_integral,
              new_dependent,
              dependent_value)
        : NULL;
    independent_at_condition = independent_value
        ? expr_substitute(
    ...
    dependent_at_condition =
        expr_simplify_owned(dependent_at_condition);
    independent_at_condition =
        expr_simplify_owned(independent_at_condition);
    constant =
        dependent_at_condition && independent_at_condition
...
static de_attempt_t de_try_linear_transformation(
    const diffequ_t *de,
    const expr_t *independent,
    const expr_t *dependent,
    ...
    equation_t **solution_out)
{
    ...
    determinant = de_linear_determinant(
        transformed_dependent, transformed_independent);
    ...
    new_dependent =
        expr_new_named_var(NUM_NAN, "__de_Y");
    x_replacement = de_inverse_x(
        transformed_dependent,
        transformed_independent,
        new_independent,
        new_dependent,
        determinant);
    y_replacement = de_inverse_y(
        transformed_dependent,
        transformed_independent,
        new_independent,
        new_dependent,
    ...
    after_x = x_replacement
        ...
        : NULL;
    transformed_rhs_raw = after_x && y_replacement
        ? expr_substitute(after_x, dependent, y_replacement)
        : NULL;
    transformed_rhs = transformed_rhs_raw
        ? expr_simplify(transformed_rhs_raw)
        : NULL;
    if (!transformed_rhs ||
        de_expr_uses(transformed_rhs, independent) ||
        de_expr_uses(transformed_rhs, dependent))
        goto cleanup;

    new_derivative = de_transformed_derivative(
        transformed_rhs,
        transformed_dependent,
        transformed_independent);
    if (new_derivative) {
        expr_t *display_derivative =
            expr_display_simplified(new_derivative);
    ...
    dependent_integral = reciprocal
        ? de_integrate_or_formal(
              reciprocal, new_dependent)
        : NULL;
    independent_integral = independent_factor
        ? de_integrate_or_formal(
              independent_factor, new_independent)
        : NULL;
    expr_free(independent_factor);
    independent_factor = NULL;
    left = dependent_integral
        ? expr_substitute(
              dependent_integral,
              new_dependent,
              transformed_dependent->expression)
        : NULL;
    right_base = independent_integral
        ? expr_substitute(
              independent_integral,
              new_independent,
              transformed_independent->expression)
        : NULL;
    constant = dependent_integral && independent_integral
        ? de_transformed_constant(
              de,
              dependent,
              transformed_dependent,
              transformed_independent,
              ...
              independent_integral)
        : NULL;
    right = right_base && constant
        ? expr_add_simplify_owned(right_base, constant)
        : NULL;
    ...
    equation_t **solution_out)
{
    ...
    de_collect_linear_forms(
        derivative_right, independent, dependent, &forms);
    for (size_t i = 0u;
         i < forms.count &&
        ...
        for (size_t j = 0u;
            ...
            if (i == j)
                continue;
            attempt = de_try_linear_transformation(
                de,
                independent,
                dependent,
                derivative_right,
