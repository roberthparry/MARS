...
static bool de_pde_collect_parameter_symbols(
    ...
    size_t *capacity)
{
    ...
    if (name && !expr_struct_eq(expression, independent) &&
        !expr_struct_eq(expression, dependent)) {
        ...
        if (unknown) {
            ...
            if (*count == *capacity) {
                ...
                expr_t **next = realloc(
                    *parameters, next_capacity * sizeof(*next));
    ...
    if (!expr_child_exprs(expression, &left, &right))
        return true;
    return de_pde_collect_parameter_symbols(
               left,
               independent,
               dependent,
               parameters,
               count,
               capacity) &&
           de_pde_collect_parameter_symbols(
               right,
               independent,
               dependent,
               parameters,
...
static expr_t **de_pde_parameter_symbols(
    const expr_t *expression,
    const expr_t *independent,
    const expr_t *dependent,
    size_t *count_out)
{
    ...
    *count_out = 0u;
    if (!de_pde_collect_parameter_symbols(
            expression,
            independent,
            dependent,
            &parameters,
...
static bool de_pde_split_parameter_factor(
    const expr_t *expression,
    const expr_t *wrt,
    expr_t **parameter_factor_out,
    expr_t **variable_factor_out)
{
    ...
    *variable_factor_out = NULL;
    ...
    if (expr_match_neg_expr(expression, &left)) {
        ...
        if (!de_pde_split_parameter_factor(
                left, wrt, &parameter_factor, &variable_factor))
            return false;
        *parameter_factor_out =
            de_simplify_unary_owned(parameter_factor, expr_neg);
    ...
    if (product || division) {
        ...
        if (!de_pde_split_parameter_factor(
                left, wrt, &left_parameter, &left_variable) ||
            !de_pde_split_parameter_factor(
                right, wrt, &right_parameter, &right_variable)) {
        ...
        *parameter_factor_out = division
            ? expr_div_simplify_owned(left_parameter, right_parameter)
            : expr_mul_simplify_owned(left_parameter, right_parameter);
        *variable_factor_out = division
            ? expr_div_simplify_owned(left_variable, right_variable)
...
static expr_t *de_pde_integrate_parameterised(
    ...
    const expr_t *wrt)
{
    ...
    if (expr_match_add_sub_expr(
            integrand, &left, &right, &subtraction)) {
        expr_t *left_integral =
            de_pde_integrate_parameterised(left, wrt);
        expr_t *right_integral =
            de_pde_integrate_parameterised(right, wrt);
        ...
        return subtraction
            ? expr_sub_simplify_owned(left_integral, right_integral)
...

    {
        ...
        if (parameter_factor && variable_integral)
            result = expr_mul_simplify_owned(
                parameter_factor, variable_integral);
        else {
...
static bool de_pde_parameter_identity_is_zero(
    const expr_t *expression,
    expr_t *const *parameters,
    size_t parameter_count)
{
    ...
    if (!is_zero) {
        ...
        is_zero = expanded_simplified &&
            expr_is_exact_zero(expanded_simplified);
    }
    ...
    for (size_t sample_index = 0u;
         sample_index < 2u * parameter_count + 5u;
         ++sample_index) {
        expr_t *sample = expr_clone(expression);

        for (size_t parameter_index = 0u;
             sample && parameter_index < parameter_count;
             ++parameter_index) {
            long sample_value =
                1L + (long)sample_index + 2L * (long)parameter_index;
            expr_t *value = expr_const_long(sample_value);
            expr_t *substituted = value
                ? expr_substitute(
                      sample, parameters[parameter_index], value)
                : NULL;
...
static bool de_pde_is_exact_one(const expr_t *expression)
{
    ...
    bool is_one = expr_match_const_value(expression, &value) &&
...
static expr_t *de_pde_cancel_verified_unit_exponentials(
    const expr_t *expression,
    expr_t *const *parameters,
    size_t parameter_count)
{
    ...
    if (expr_match_exp_expr(expression, &left) &&
        de_pde_parameter_identity_is_zero(
            left, parameters, parameter_count)) {
        return expr_const_one();
    }
    if (expr_match_add_sub_expr(
            expression, &left, &right, &subtraction)) {
        left_result = de_pde_cancel_verified_unit_exponentials(
            left, parameters, parameter_count);
        right_result = de_pde_cancel_verified_unit_exponentials(
            right, parameters, parameter_count);
        ...
        return subtraction
            ? expr_sub_simplify_owned(left_result, right_result)
...
static bool de_pde_uses_any_parameter(
    const expr_t *expression,
    expr_t *const *parameters,
    size_t parameter_count)
{
...
static bool de_pde_parameter_linear_steps(
    const expr_t *independent,
    const expr_t *dependent,
    expr_t *const *parameters,
    size_t parameter_count,
    ...
    const expr_t *coefficient_integral,
    const expr_t *integrating_factor,
    const expr_t *weighted_forcing,
    const expr_t *forcing_integral,
    const expr_t *arbitrary,
    const expr_t *solved_right,
    char **steps_out,
    char **steps_tex_out)
{
    ...
    *steps_out = NULL;
    *steps_tex_out = NULL;
    if (!independent || !dependent || !parameters ||
        parameter_count == 0u || !coefficient || !forcing ||
        !coefficient_integral || !integrating_factor ||
        !weighted_forcing || !forcing_integral || !arbitrary ||
        !solved_right)
        return false;
    ...
    coefficient_integral_text =
        expr_to_string(coefficient_integral, style_UNBOUND);
    integrating_factor_text =
        expr_to_string(integrating_factor, style_UNBOUND);
    weighted_forcing_text =
        expr_to_string(weighted_forcing, style_UNBOUND);
    forcing_integral_text =
        expr_to_string(forcing_integral, style_UNBOUND);
    arbitrary_text = expr_to_string(arbitrary, style_UNBOUND);
    solved_right_text = expr_to_string(solved_right, style_UNBOUND);

    independent_symbol = expr_new_named_const(
    ...
    for (size_t i = 0u;
         parameter_text_builder && parameter_tex_builder &&
             i < parameter_count;
         ++i) {
        ...
        if (!text || !tex ||
            (i > 0u &&
             (string_append_cstr(parameter_text_builder, ", ") != 0 ||
    ...
    parameters_text = parameter_text_builder
        ? strdup(string_c_str(parameter_text_builder))
        : NULL;
    parameters_tex = parameter_tex_builder
        ? strdup(string_c_str(parameter_tex_builder))
        : NULL;

    if (!independent_text || !dependent_text || !parameters_text ||
        !coefficient_text || !forcing_text ||
        !coefficient_integral_text || !integrating_factor_text ||
        !weighted_forcing_text || !forcing_integral_text ||
        !arbitrary_text || !solved_right_text || !independent_tex ||
        !dependent_tex || !parameters_tex || !coefficient_tex ||
        !forcing_tex || !coefficient_integral_tex ||
        !integrating_factor_tex || !weighted_forcing_tex ||
        !forcing_integral_tex || !arbitrary_tex || !solved_right_tex)
        goto cleanup;
    ...
    if (!steps || !steps_tex ||
        ...
        string_append_format(
            ...
            coefficient_tex,
            independent_tex,
            coefficient_integral_tex,
            coefficient_tex,
            independent_tex,
            integrating_factor_tex,
            independent_tex,
            coefficient_tex,
            dependent_tex,
            independent_tex,
            ...
            dependent_tex,
            forcing_tex,
            weighted_forcing_tex,
            dependent_tex,
            forcing_integral_tex,
            arbitrary_tex,
            dependent_tex,
            solved_right_tex) < 0)
        goto cleanup;
    ...
    char **steps_tex_out)
{
    ...
    *steps_out = NULL;
    *steps_tex_out = NULL;
    if (!de || !de->partial_derivative_input ||
        de->condition_count != 0u || !independent || !dependent ||
        !derivative_right || !expr_symbol_name(independent))
        return DE_ATTEMPT_NOT_MATCHED;

    parameters = de_pde_parameter_symbols(
        derivative_right,
        independent,
        dependent,
        &parameter_count);
    ...
    coefficient = de_simplify_unary_owned(
        dependent_coefficient, expr_neg);
    dependent_coefficient = NULL;
    coefficient_integral = de_pde_integrate_parameterised(
    ...
    weighted_forcing = integrating_factor
        ? expr_mul_simplify_owned(
              expr_clone(integrating_factor), expr_clone(forcing))
        : NULL;
    forcing_integral = weighted_forcing
        ? de_pde_integrate_parameterised(
              weighted_forcing, independent)
        : NULL;
    arbitrary = parameter_count == 1u
        ? expr_new_arbitrary_function("F", parameters[0])
        : expr_new_arbitrary_function_n(
              "F", parameter_count, parameters);
    negative_coefficient_integral = coefficient_integral
        ? de_simplify_unary_owned(
              expr_clone(coefficient_integral), expr_neg)
        : NULL;
    inverse_integrating_factor = negative_coefficient_integral
        ? expr_exp(negative_coefficient_integral)
        : NULL;
    particular = forcing_integral && inverse_integrating_factor
        ? expr_mul_simplify_owned(
              expr_clone(forcing_integral),
              expr_clone(inverse_integrating_factor))
        : NULL;
    if (particular && de_pde_uses_any_parameter(
            coefficient, parameters, parameter_count)) {
        expr_t *expanded_particular = expr_display_expanded(particular);
        expr_t *simplified_particular = expanded_particular
            ? de_pde_cancel_verified_unit_exponentials(
                  expanded_particular, parameters, parameter_count)
            : NULL;
        expr_t *final_particular = simplified_particular
            ? expr_display_expanded(simplified_particular)
            : NULL;
    ...
    homogeneous = arbitrary && inverse_integrating_factor
        ? expr_mul_simplify_owned(
              ...
              expr_clone(inverse_integrating_factor))
        : NULL;
    solved_right = particular && homogeneous
        ? expr_add(particular, homogeneous)
        : NULL;
    if (!coefficient || !coefficient_integral ||
        !integrating_factor || !weighted_forcing ||
        !forcing_integral || !arbitrary || !solved_right)
        goto cleanup;

    coefficient_derivative =
        expr_create_deriv(coefficient_integral, independent);
    coefficient_check_raw = coefficient_derivative
        ? expr_sub(coefficient_derivative, coefficient)
        : NULL;
    forcing_derivative =
        expr_create_deriv(forcing_integral, independent);
    forcing_check_raw = forcing_derivative
        ? expr_sub(forcing_derivative, weighted_forcing)
        : NULL;
    if (!de_pde_parameter_identity_is_zero(
            coefficient_check_raw, parameters, parameter_count) ||
        !de_pde_parameter_identity_is_zero(
            forcing_check_raw, parameters, parameter_count))
        goto cleanup;

    *solution_out = de_pde_solution_equation(dependent, solved_right);
    if (!*solution_out ||
        (include_steps && !de_pde_parameter_linear_steps(
            independent,
            dependent,
            parameters,
            parameter_count,
