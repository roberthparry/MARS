...
static bool de_pde_collect_dependent_derivatives(
    ...
    size_t capacity)
{
    ...
    if (expr_is_formal_derivative(expression) &&
        de_pde_same_symbolic_form(
            expr_formal_derivative_dependent(expression), dependent)) {
    ...
    if (!expr_child_exprs(expression, &left, &right))
        return true;
    return de_pde_collect_dependent_derivatives(
               left, dependent, derivatives, count, capacity) &&
           de_pde_collect_dependent_derivatives(
               right, dependent, derivatives, count, capacity);
}

static expr_t *de_pde_apply_spatial_operator(
    const expr_t *operator_expression,
    const expr_t *dependent,
    const expr_t *initial)
{
    expr_t *applied = expr_substitute(
        operator_expression, dependent, initial);
...
static bool de_pde_stationary_rate_matches_at_point(
    const expr_t *rate_expression,
    const diffequ_t *de,
    size_t time_index,
    ...
    size_t sample_index)
{
    ...
    for (size_t i = 0u; sample && i < de->independent_count; ++i) {
        ...
        substituted = point
            ? expr_substitute(sample, de->independent_vars[i], point)
            : NULL;
        expr_free(point);
        expr_free(sample);
        sample = substituted;
    ...
    difference_raw = sample
        ? expr_sub(sample, expected_rate)
        : NULL;
    ...
    if (!matches && difference) {
        ...
        matches = num_is_finite(magnitude) &&
                  num_to_double(magnitude) < 1.0e-20;
...
static const expr_t *de_pde_initial_time_point(
    const diffequ_t *de,
    size_t time_index)
{
    ...
    for (size_t point_index = 0u;
         point_index < de->condition_point_counts[0];
         ++point_index) {
        const expr_t *point = de->condition_points[0][point_index];
        bool is_free_spatial_coordinate = false;

        for (size_t independent_index = 0u;
             independent_index < de->independent_count;
             ++independent_index) {
            if (independent_index != time_index &&
                de_pde_same_symbolic_form(
                    point, de->independent_vars[independent_index])) {
...
static bool de_pde_verify_stationary_rate(
    const expr_t *rate_expression,
    const diffequ_t *de,
    size_t time_index,
    const expr_t *expected_rate)
{
    ...
    for (size_t sample = 0u; sample < sample_count; ++sample)
        if (!de_pde_stationary_rate_matches_at_point(
                rate_expression, de, time_index, expected_rate, sample))
            return false;
    return true;
}

static bool de_pde_set_stationary_steps(
    diffequ_solve_result_t *result,
    const expr_t *dependent,
    const expr_t *time_coordinate,
    const expr_t *time_point,
    const expr_t *time_coefficient,
    const expr_t *spatial_operator,
    const expr_t *initial,
    const expr_t *rate,
    const equation_t *solution)
{
    ...
    bool success = u && t && t0 && a && h && u0 && lambda &&
        u_tex && t_tex && t0_tex && a_tex && h_tex && u0_tex &&
        lambda_tex && solution_text && solution_tex && steps && steps_tex &&
        string_append_format(
            ...
            "Hence %s = %s₀·exp(λ(%s − %s)):\n"
            "      %s",
            u, t, u, a, u, h, t, t0, u, u0,
            u, u, u, lambda, u, u, t, t0,
        ...
        string_append_format(
            ...
            "&%s"
            "\\end{aligned}",
            u_tex, t_tex, u_tex, a_tex, u_tex, h_tex,
            u_tex, u0_tex, t_tex, t0_tex,
            u_tex, u_tex, lambda_tex,
            u_tex, u_tex, t_tex, t0_tex,
...
static diffequ_solve_result_t *de_pde_solve_stationary_eigenfunction(
    const diffequ_t *de,
    ...
    bool include_steps)
{
    ...
    if (!de || !residual || de->condition_count != 1u ||
        de->condition_point_counts[0] != de->independent_count)
        goto cleanup;
    for (size_t i = 0u; i < de->independent_count; ++i) {
        ...
        if (!de_pde_collect_dependent_derivatives(
                residual,
                equ_lhs(de->conditions[0]),
                derivatives,
                &derivative_count,
                64u))
            goto cleanup;
        for (size_t j = 0u; j < derivative_count; ++j)
            if (expr_formal_derivative_order(derivatives[j]) == 1u &&
                expr_struct_eq(
                    expr_formal_derivative_wrt_at(derivatives[j], 0u),
                    de->independent_vars[i])) {
    ...
    if (!dependent || !time_derivative ||
        !de_pde_same_symbolic_form(
            equ_lhs(de->conditions[0]), dependent) ||
        !de_linear_decompose(
            residual,
            time_derivative,
            &time_coefficient,
    ...
    if (!initial_time)
        goto cleanup;
    applied_operator = de_pde_apply_spatial_operator(
        spatial_operator, dependent, initial);
    denominator = expr_mul_simplify_owned(
        expr_clone(time_coefficient), expr_clone(initial));
    rate_raw = applied_operator && denominator
        ? expr_div_simplify_owned(
              expr_negate_owned(applied_operator), denominator)
        : NULL;
    applied_operator = NULL;
    denominator = NULL;
    rate_sample = expr_clone(rate_raw);
    for (size_t i = 0u;
         rate_sample && i < de->independent_count;
         ++i) {
        expr_t *point;
        expr_t *substituted;
        ...
        if (i == time_index)
            continue;
        point = expr_const_long(i == 0u ||
            (time_index == 0u && i == 1u) ? 1L : 0L);
        substituted = point
            ? expr_substitute(
                  rate_sample, de->independent_vars[i], point)
            : NULL;
        expr_free(point);
    ...
    if (!rate || !de_pde_verify_stationary_rate(
            rate_raw, de, time_index, rate))
        goto cleanup;
    for (size_t i = 0u; i < de->independent_count; ++i)
        if (de_expr_uses(rate, de->independent_vars[i]))
            goto cleanup;
    time_offset = expr_sub_simplify_owned(
        expr_clone(de->independent_vars[time_index]),
        expr_clone(initial_time));
    phase_exponent = time_offset
        ? expr_mul_simplify_owned(expr_clone(rate), time_offset)
        : NULL;
    time_offset = NULL;
    phase = phase_exponent ? expr_exp(phase_exponent) : NULL;
    right = phase
        ? expr_mul_simplify_owned(expr_clone(initial), phase)
        : NULL;
    phase = NULL;
    ...
    solution = NULL;
    if (include_steps && !de_pde_set_stationary_steps(
            result,
            dependent,
            de->independent_vars[time_index],
            initial_time,
            time_coefficient,
            spatial_operator,
            initial,
...
static bool de_pde_cartesian_laplace_steps(
    const diffequ_t *de,
    const equation_t *solution,
    char **steps_out,
    char **steps_tex_out)
{
    ...
    if (steps_tex_out)
        *steps_tex_out = NULL;
    if (!de || de->independent_count != 2u || !dependent || !right ||
        !steps_out || !steps_tex_out)
        return false;
    first_coordinate = de->independent_vars[0];
    second_coordinate = de->independent_vars[1];
    if (!first_coordinate || !second_coordinate ||
        !expr_symbol_name(first_coordinate) ||
        !expr_symbol_name(second_coordinate))
        return false;

    imaginary_unit = expr_new_named_const(NUM_I, "i");
    first_coordinate_symbol = expr_new_named_const(
        NUM_NAN, expr_symbol_name(first_coordinate));
    second_coordinate_symbol = expr_new_named_const(
        NUM_NAN, expr_symbol_name(second_coordinate));
    imaginary_second = imaginary_unit
        ? expr_mul_simplify_owned(
              expr_clone(imaginary_unit), expr_clone(second_coordinate))
        : NULL;
    positive_coordinate = imaginary_second
        ? expr_add(first_coordinate, imaginary_second)
        : NULL;
    negative_coordinate = imaginary_second
        ? expr_sub(first_coordinate, imaginary_second)
        : NULL;
    first_derivative_raw = expr_create_deriv(right, first_coordinate);
    first_derivative = first_derivative_raw
        ? expr_simplify(first_derivative_raw)
        : NULL;
    first_second_derivative_raw = first_derivative
        ? expr_create_deriv(first_derivative, first_coordinate)
        : NULL;
    first_second_derivative = first_second_derivative_raw
        ? expr_simplify(first_second_derivative_raw)
        : NULL;
    second_derivative_raw = expr_create_deriv(right, second_coordinate);
    second_derivative = second_derivative_raw
        ? expr_simplify(second_derivative_raw)
        : NULL;
    second_second_derivative_raw = second_derivative
        ? expr_create_deriv(second_derivative, second_coordinate)
        : NULL;
    second_second_derivative = second_second_derivative_raw
        ? expr_simplify(second_second_derivative_raw)
        : NULL;
    ...
    if (!positive_coordinate || !negative_coordinate ||
        !first_derivative || !first_second_derivative ||
        !second_derivative || !second_second_derivative ||
        !laplacian || !expr_is_exact_zero(laplacian))
        goto cleanup;

#define DE_LAPLACE_TEXT(name, expression) \
    name = expr_to_string((expression), style_UNBOUND)
#define DE_LAPLACE_TEX(name, expression) \
    name = expr_to_tex_body((expression))
...
#undef DE_LAPLACE_TEXT
    if (!dependent_text || !first_coordinate_text ||
        !second_coordinate_text || !positive_coordinate_text ||
        !negative_coordinate_text || !right_text ||
        !first_second_derivative_text ||
        !second_second_derivative_text ||
        !dependent_tex || !first_coordinate_tex ||
        !second_coordinate_tex || !positive_coordinate_tex ||
        !negative_coordinate_tex || !right_tex ||
        !first_second_derivative_tex ||
        !second_second_derivative_tex)
        goto cleanup;
    ...
    if (!steps || !steps_tex ||
        string_append_format(
            ...
            "           = (%s) + (%s)\n"
            "           = 0",
            positive_coordinate_text,
            negative_coordinate_text,
            dependent_text,
            right_text,
            dependent_text,
            ...
            second_coordinate_text,
            first_second_derivative_text,
            second_second_derivative_text) < 0 ||
        string_append_format(
            steps_tex,
            "\\begin{aligned}[t]"
            ...
            "&=0"
            "\\end{aligned}",
            positive_coordinate_tex,
            negative_coordinate_tex,
            dependent_tex,
            right_tex,
            dependent_tex,
            first_coordinate_tex,
            first_second_derivative_tex,
            dependent_tex,
            second_coordinate_tex,
            second_second_derivative_tex,
            dependent_tex,
            dependent_tex,
            first_coordinate_tex,
...
diffequ_solve_result_t *de_pde_solve_two_variable(
    ...
    bool include_steps)
{
    static const char polar_laplace_steps[] =
        "Set the complex polar coordinates:\n"
    ...
    static const char polar_laplace_steps_tex[] =
        "\\begin{aligned}[t]"
    ...
    polar_laplace = residual
        ? de_pde_attempt_polar_laplace(
              de, residual, &polar_laplace_solution)
        : DE_ATTEMPT_FAILED;
    if (polar_laplace == DE_ATTEMPT_SOLVED) {
        result = de_solve_result_new(
            ...
            "Laplace equation");
        if (!result ||
            (include_steps && de_solve_result_set_steps(
                result, polar_laplace_steps) != 0) ||
            (include_steps && de_solve_result_set_steps_tex(
                result, polar_laplace_steps_tex) != 0) ||
            de_solve_result_append(
                result, polar_laplace_solution) != 0) {
            de_solve_result_free(result);
            result = NULL;
            goto cleanup;
        }
    ...
    laplace = residual
        ? de_pde_attempt_laplace(de, residual, &laplace_solution)
        : DE_ATTEMPT_FAILED;
    if (laplace == DE_ATTEMPT_SOLVED) {
        if (include_steps && !de_pde_cartesian_laplace_steps(
                de,
                laplace_solution,
                &laplace_steps,
                &laplace_steps_tex)) {
        ...
        if (!result ||
            (include_steps &&
             de_solve_result_set_steps(result, laplace_steps) != 0) ||
            (include_steps &&
             de_solve_result_set_steps_tex(result, laplace_steps_tex) != 0) ||
    ...
    stationary = de_pde_solve_stationary_eigenfunction(
        de, residual, include_steps);
    if (stationary) {
        result = stationary;
        goto cleanup;
    }
    ...
    result = de_solve_result_new(
        ...
        polar_laplace == DE_ATTEMPT_FAILED
            ...
            : laplace == DE_ATTEMPT_FAILED
            ? "failed to complete the Laplace-equation solution"
            : transport == DE_ATTEMPT_FAILED ||
                  characteristics == DE_ATTEMPT_FAILED
