...
static expr_t *de_linear_constant(const diffequ_t *de, const expr_t *dependent, const expr_t *independent,
                                  ...
                                  const expr_t *integral)
{
    ...
    scaled_value = factor_at_point ? expr_mul_simplify_owned(expr_clone(value), factor_at_point) : NULL;
    if (factor_at_point)
...
de_attempt_t de_attempt_linear(const diffequ_t *de, const expr_t *independent, const expr_t *dependent,
                               const expr_t *derivative_right, equation_t **solution_out)
{
    ...
    if (!de_linear_decompose(derivative_right, dependent, &dependent_coefficient, &forcing))
        goto cleanup;
    if (de_expr_uses(dependent_coefficient, dependent) || de_expr_uses(forcing, dependent))
        goto cleanup;
    attempt = DE_ATTEMPT_FAILED;

    coefficient = de_simplify_unary_owned(dependent_coefficient, expr_neg);
    dependent_coefficient = NULL;
    coefficient_integral = de_integrate_or_formal(coefficient, independent);
    integrating_factor = coefficient_integral ? expr_exp(coefficient_integral) : NULL;
    weighted_forcing = integrating_factor ? expr_mul_simplify_owned(expr_clone(integrating_factor), forcing) : NULL;
    ...
    constant =
        de_linear_constant(de, dependent, independent, coefficient_integral, integrating_factor, forcing_integral);
    numerator = constant ? expr_add_simplify_owned(forcing_integral, constant) : NULL;
    if (constant) {
        forcing_integral = NULL;
        constant = NULL;
    }
    solved_right = numerator ? expr_div_simplify_owned(numerator, integrating_factor) : NULL;
    if (numerator) {
        numerator = NULL;
