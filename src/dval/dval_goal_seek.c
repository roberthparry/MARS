...
{
    size_t scaled = digits > 0u ? digits * 4u : DV_GOAL_DEFAULT_ITERATIONS;

    return scaled > DV_GOAL_DEFAULT_ITERATIONS
         ? scaled
         : DV_GOAL_DEFAULT_ITERATIONS;
}

static size_t goal_precision_digits(const dv_goal_seek_options_t *options)
{
    if (options && options->precision_digits > 0u)
        return options->precision_digits;
    return DV_GOAL_DEFAULT_DIGITS;
}
...
static void goal_result_reset(dv_goal_seek_result_t *result)
{
    if (!result)
        return;
    result->expr = NULL;
...
static int goal_eval_residual(dval_t *expr,
                              number_t target,
                              number_t *value_out,
...
static int goal_eval_derivative_residual(dval_t *expr,
                                         dval_t *var,
                                         number_t target,
                                         number_t *value_out,
                                         number_t *residual_out,
                                         number_t *derivative_out)
{
    ...
    vars[0] = var;
    if (dv_eval_derivatives(expr, 1u, vars, &value, &deriv) != 0) {
        num_destroy(&deriv);
        num_destroy(&value);
        return -1;
    }
...
static int goal_finish(dval_t *expr,
                       ...
                       dv_goal_seek_result_t *result)
{
    ...
    result->expr = options && options->simplify_result ? dv_simplify(expr) : NULL;
    if (!result->expr) {
        dv_retain(expr);
        result->expr = expr;
    }
...
static int goal_collect_variables(dval_bindings_t *bindings, dval_t ***vars_out)
{
    dval_t **vars;
    size_t count = 0u;
    ...
    for (size_t i = 0u; i < bindings->count; ++i) {
        if (!bindings->entries[i].is_constant)
            vars[count++] = bindings->entries[i].dval;
    }
...
static int goal_solve_real_one(dval_t *expr,
                               dval_t *var,
                               number_t target,
                               number_t tolerance,
                               size_t digits,
...
static void goal_set_vars(dval_t **vars,
                          number_t *values,
                          size_t nvars,
                          size_t digits)
{
...
static int goal_eval_real_gradient(dval_t *expr,
                                   dval_t **vars,
                                   size_t nvars,
                                   number_t target,
                                   number_t *residual_out,
                                   number_t *derivs_out)
{
    const dval_t **var_refs = calloc(nvars, sizeof(*var_refs));
    number_t value = (number_t){0};
    number_t residual;
    int rc = -1;
    ...
    if (dv_eval_derivatives(expr, nvars, var_refs, &value, derivs_out) != 0)
        goto cleanup;
...
static int goal_solve_real_multi(dval_t *expr,
                                 dval_t **vars,
                                 size_t nvars,
                                 number_t target,
                                 number_t tolerance,
