...
static number_t equ_div_long(number_t value, long divisor)
{
...
static number_t equ_cubic_shifted_root(number_t term, number_t shift)
{
...
static number_t equ_cubic_real_cuberoot(number_t value)
{
...
static number_t equ_cubic_omega(bool conjugate)
{
...
static number_t equ_cubic_discriminant(number_t p, number_t q)
{
    number_t q_half = equ_div_long(q, 2L);
    number_t p_third = equ_div_long(p, 3L);
                                                         ...
                                                         equation_solutions_t *solutions)
{
    ...
    if (num_is_zero(p)) {
        ...
        root = equ_cubic_shifted_root(zero_term, shift);
...

    {
        ...
        root = equ_cubic_shifted_root(first_term, shift);
        rc = equ_append_distinct_root(wrt, root, seen, seen_count, solutions);
        num_destroy(&root);
                                            ...
                                            equation_solutions_t *solutions)
{
    ...
    number_t theta = equ_div_long(theta_raw, 3L);
    number_t two_pi = num_mul_long(NUM_PI, 2L);
    number_t turn = equ_div_long(two_pi, 3L);
    int rc = 0;
    ...
    for (size_t i = 0u; i < 3u && rc == 0; ++i) {
        ...
        number_t root = equ_cubic_shifted_root(term, shift);
                                               ...
                                               equation_solutions_t *solutions)
{
    ...
    if (rc == 0) {
        ...
        root = equ_cubic_sum_root(omega_u, omega_v, shift);
        rc = equ_append_distinct_root(wrt, root, seen, seen_count, solutions);
        num_destroy(&root);
        num_destroy(&omega_v);
        num_destroy(&omega_u);
                                       ...
                                       expr_t **v_out)
{
    ...
    *v_out = equ_cubic_expr_cuberoot(v_arg);
    ok = *u_out && *v_out;
                                             ...
                                             equation_solutions_t *solutions)
{
    ...
    p = equ_cubic_symbolic_p(cubic, quadratic, linear);
    ...
    omega = equ_cubic_omega_expr(false);
    omega_conj = equ_cubic_omega_expr(true);
    root = equ_cubic_symbolic_root(u, v, shift, omega, omega_conj);
    if (!root || equ_append_solution_expr(wrt, root, solutions) != 0) {
    ...
    root = equ_cubic_symbolic_root(u, v, shift, omega_conj, omega);
    if (!root || equ_append_solution_expr(wrt, root, solutions) != 0) {
        equ_solutions_clear(solutions);
        goto cleanup;
    }
